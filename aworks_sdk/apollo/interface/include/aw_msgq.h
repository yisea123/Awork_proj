 ...
 *     判断调用结果: AW_OK -成功获取消息；-AW_ENXIO -在等待的时候，消息队列被终止
 *
 *   - #AW_MSGQ_NO_WAIT \n
 ...
 *                       将毫秒转换为节拍数; #AW_MSGQ_NO_WAIT 为无等待;
 *                       #AW_MSGQ_WAIT_FOREVER 为一直等待
 *
 * \retval   AW_OK      成功接收消息
 ...
 *     判断调用结果: AW_OK -成功发送消息；-AW_ENXIO -在等待的时候，消息队列被终止
 *
 *   - #AW_MSGQ_NO_WAIT \n
 *     立即返回，不管是否成功发送到消息。通过返回值来判断调用结果:
 *     AW_OK -成功发送消息; -TIME -发送超时; -AW_ENXIO -在等待的时候,消息队列被终止
 *
 * 除了 #AW_MSGQ_WAIT_FOREVER 外，其它的负值都是非法的。调用者需要保证指定的超时
 * 值为正数。
 *
 ...
 * \brief 终止消息队列
 *
 * 终止消息队列。任何等待此消息队列的任务将会解阻塞，并返回 -AW_ENXIO 。此函数不会
 * 释放消息队列相关的任何内存。
 *
 ...
 * \retval  FALSE   无效队列
 */
bool_t aw_msgq_valid (aw_msgq_id_t msgq_id);

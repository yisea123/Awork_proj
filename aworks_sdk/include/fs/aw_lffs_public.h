 ...
 * \note Be careful: it's part of the physical format (see: aw_lffs_file_info_tSt.name)
 *    !!DO NOT CHANGE IT AFTER FILE SYSTEM IS FORMATED!!
 */
#define AW_LFFS_MAX_FILENAME_LENGTH         128

/** \note 8-bits attr goes to lffs_dirent::d_type */
#define AW_LFFS_FILE_ATTR_DIR       (1 << 7)    //!< attribute for directory
#define AW_LFFS_FILE_ATTR_WRITE     (1 << 0)    //!< writable
 ...
 * \brief lffs tag, 9 bytes, will be store in page spare area.
 */
struct aw_lffs_tag_store {
    uint32_t valid:8;        //!< 0: valid, 1: invalid
    uint32_t dirty:1;        //!< 0: dirty, 1: clear
    uint32_t type:2;         //!< block type: #AW_LFFS_TYPE_DIR, #AW_LFFS_TYPE_FILE, #AW_LFFS_TYPE_DATA
    ...
    uint32_t reserved:4;     //!< reserved, for LFFS2
    uint32_t tag_ecc:12;     //!< tag ECC
};

#define AW_LFFS_TAG_ECC_DEFAULT (0xFFF) //!< 12-bit '1'
 ...
 */
struct aw_lffs_tags {
    struct aw_lffs_tag_store s;       /* store must be the first member */
...
/** aw_lffs_tags_tSt.dirty */
#define AW_LFFS_TAG_VALID       0xEE
#define AW_LFFS_TAG_INVALID     1

/** aw_lffs_tags_tSt.valid */
#define AW_LFFS_TAG_DIRTY       0
#define AW_LFFS_TAG_CLEAR       1

#define AW_LFFS_TAG_IS_DIRTY(tag) (*((uint32_t *) &((tag)->s)) != 0xFFFFFFFF)    // tag is dirty if first 4 bytes not all 0xFF
#define AW_LFFS_TAG_IS_VALID(tag) ((tag)->s.valid == AW_LFFS_TAG_VALID)
#define AW_LFFS_TAG_IS_SEALED(tag) ((tag)->seal_byte != 0xFF)

#define AW_LFFS_TAG_IS_GOOD(tag) (AW_LFFS_TAG_IS_SEALED(tag) && AW_LFFS_TAG_IS_VALID(tag))
...
bool_t aw_lffs_is_src_newer_than_obj (int src, int obj);
...
#define AW_LFFS_MSG_OS              0x00000008
...
void aw_lffs_debug_message (uint32_t mask, const char *prefix, const char *suffix, int line, const char *errFmt, ...);
...
bool_t aw_lffs_assert(bool_t expr, const char *fmt, ...);

#else
#define aw_lffs_msg_raw(mask, fmt, ...)  aw_lffs_debug_message(mask, NULL, NULL, -1, fmt, ##__VA_ARGS__)

#define aw_lffs_msg(mask, fmt, ...)  aw_lffs_debug_message(mask, PFX, AW_LFFS_TENDSTR, __LINE__, fmt, ##__VA_ARGS__)
...
#define aw_lffs_assert(expr, msg, ...)  \
        ((expr) ? TRUE : (aw_lffs_assert_call(__FILE__, __LINE__, msg, ## __VA_ARGS__), FALSE))
...
aw_err_t aw_lffs_page_spare_load (aw_lffs_device_t *dev, int block, int page, aw_lffs_tags_t *tag);
aw_err_t aw_lffs_page_spare_write (aw_lffs_device_t *dev, int block, int page, aw_lffs_tags_t *tag);
aw_err_t aw_lffs_page_valid_make (aw_lffs_device_t *dev, int block, int page, aw_lffs_tags_t *tag);
bool_t aw_lffs_block_is_bad (aw_lffs_device_t *dev, aw_lffs_block_info_t *bc);

/********************************** Public defines *****************************/
/**
 * \def AW_LFFS_ALL_PAGES
 * \brief AW_LFFS_ALL_PAGES if this value presented, that means the objects are all pages in the block
 */
...
aw_err_t aw_lffs_new_block (aw_lffs_device_t *dev, uint16_t block, aw_lffs_tags_t *tag, aw_lffs_buf_t *buf);
aw_err_t aw_lffs_block_recover (aw_lffs_device_t *dev, aw_lffs_block_info_t *old, uint16_t newBlock);
...
uint16_t aw_lffs_best_page_find (aw_lffs_device_t *dev, aw_lffs_block_info_t *bc, uint16_t page);
uint16_t aw_lffs_first_free_page_find (aw_lffs_device_t *dev, aw_lffs_block_info_t *bc, uint16_t pageFrom);
uint16_t aw_lffs_valid_page_find (aw_lffs_device_t *dev, aw_lffs_block_info_t *bc, uint16_t page);

uint8_t aw_lffs_sum8_make (const void *p, int len);
uint16_t aw_lffs_sum16_make (const void *p, int len);
aw_err_t aw_lffs_new_file_create (aw_lffs_device_t *dev, uint16_t parent, uint16_t serial, aw_lffs_block_info_t *bc, aw_lffs_file_info_t *fi);

int aw_lffs_file_length_get (aw_lffs_device_t *dev, aw_lffs_block_info_t *bc, uint8_t type);
bool_t aw_lffs_page_is_erased (aw_lffs_device_t *dev, aw_lffs_block_info_t *bc, uint16_t page);
int aw_lffs_free_page_num_get (aw_lffs_device_t *dev, aw_lffs_block_info_t *bc);
bool_t aw_lffs_data_block_regu_is_full (aw_lffs_device_t *dev, aw_lffs_block_info_t *bc);
bool_t aw_lffs_block_is_used (aw_lffs_device_t *dev, aw_lffs_block_info_t *bc);
bool_t aw_lffs_block_pages_is_all_used (aw_lffs_device_t *dev, aw_lffs_block_info_t *bc);

int aw_lffs_block_time_stamp_get (aw_lffs_device_t *dev, aw_lffs_block_info_t *bc);
...
aw_err_t aw_lffs_mini_header_load (aw_lffs_device_t *dev, int block, uint16_t page, struct aw_lffs_mini_header *header);

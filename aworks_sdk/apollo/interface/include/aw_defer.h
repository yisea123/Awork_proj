 ...
 *  {
 *      if (aw_defer_job_is_usable(&g_myjob) == TRUE) {
 *          // 节点可用，将其添加到队列中
 ...
 * \endcode
 */
bool_t aw_defer_job_is_usable (struct aw_defer *p_this, struct aw_defer_djob *p_job);

/**
 * \brief 将一个工作节点添加至延迟作业中处理，节点添加后只能被执行一次（即会被自动清除）,
 *        若当前工作节点已添加，那直接返回错误
 *

 ...
 */
#define CLK_SET_RATE_GATE       AW_BIT(0)   /* must be gated across rate change */
#define CLK_SET_PARENT_GATE     AW_BIT(1)   /* must be gated across re-parent */
#define CLK_SET_RATE_PARENT     AW_BIT(2)   /* propagate rate change up one level */
#define CLK_IGNORE_UNUSED       AW_BIT(3)   /* do not gate even if unused */
#define CLK_IS_ROOT             AW_BIT(4)   /* root clk, has no parent */
#define CLK_IS_BASIC            AW_BIT(5)   /* Basic clk, can't do a to_clk_foo() */
#define CLK_GET_RATE_NOCACHE    AW_BIT(6)   /* do not use the cached clk rate */
#define CLK_SET_RATE_NO_REPARENT AW_BIT(7) /* don't re-parent on rate change */
#define CLK_GET_ACCURACY_NOCACHE AW_BIT(8) /* do not use the cached clk accuracy */
/*
...
// 时钟的频率和父时钟不能被改变，主要供内部使用，用于标记不能随便修改的时钟
// 如DDR时钟及其所有父时钟，以阻止意外的修改
...
/**
 * struct clk_user_fixed_rate - 用户固定频率时钟，

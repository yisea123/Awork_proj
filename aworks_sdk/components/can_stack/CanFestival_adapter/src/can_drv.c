...
#define __COP_CAN_RECV_MSG_TIMEOUT     10    /* CAN报文接收超时时间 */
#define __COP_CAN_SEND_MSG_TIMEOUT     300   /* CAN报文发送超时时间 */

#define __COP_MSG(x) AW_INFOF(("CAN : ")); AW_INFOF(x)

/* canopen 实例 */
typedef struct __cop_inst {
    bool_t        used; /* 该实例是否已经被使用 */
    int           chn;  /* 该实例对应的CAN通道 */
    AW_TASK_DECL (recv_task, __COP_RECV_TASK_STACK_SIZE); /* 协议栈接收任务 */
    CO_Data      *p_data;                                 /* 协议栈数据 */

    volatile aw_can_bus_status_t status;                  /* CAN总线状态 */
} __cop_inst_t;
...
UNS8 canSend (CAN_PORT p_port, Message *p_message)
{
    ...
    EnterMutex();
    p_cop_inst = p_port;
    if (p_cop_inst == NULL || p_message == NULL || p_cop_inst->used == FALSE) {
        ret = -1;
    }
    chn = p_cop_inst->chn;
    LeaveMutex();
...
static void __cop_can_recv (void *p_arg)
{
    ...
    AW_FOREVER {
        ...
        EnterMutex();
        if (p_cop_inst->used == TRUE || p_cop_inst->p_data != NULL) {

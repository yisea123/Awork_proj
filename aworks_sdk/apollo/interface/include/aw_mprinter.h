...
/**
 * \file
 * \brief 微型打印机通用标准接口，参考EPSON手册对微型打印机大多数命令进行封装。用户可根据
 *        应用扩展特殊的指令
 * \internal
 ...
 * \param[in] font_type      : 字符打印方式，AW_MPRINTER58_XX6B_FONT_TYPE_*，可
 *                             多个宏组合使用，参见\ref grp_aw_mprinter_font_type
 *
 * \retval AW_OK             : 成功
 * \retval -AW_EINVAL        : 无效参数
 * \retval -AW_EBADF         : 设备描述符损坏
...
/**
 * \brief 设置绝对打印位置（此指令只对本行有效，换行后打印位置复位为打印起始位置）
 *
 * \param[in] fd             : 微打设备描述符
 ...
 * \retval -AW_ENOSYS        : 该类型微型打印机不支持此功能
 *
 * \note 可参考EPSON微打指令GS $ Set absolute vertical print position in page mode
 */
 ...
 * \retval -AW_ENOSYS        : 该类型微型打印机不支持此功能
 *
 * \note 可参考EPSON微打指令GS T Set print position to the beginning of print line
 */
 ...
 * \retval -AW_ENOSYS        : 该类型微型打印机不支持此功能
 *
 * \note 可参考EPSON微打指令GS \ Set relative vertical print position in page mode
 */
 ...
 *
 * \param[in] fd             : 微打设备描述符
 * \param[in] type           : 打印模式， 参见\ref grp_aw_mprinter_kanjichar_mode
 *
 * \retval AW_OK             : 成功
 * \retval -AW_EBADF         : 设备描述符损坏
 * \retval -AW_ENOSYS        : 该类型微型打印机不支持此功能
 ...
 * \retval -AW_ENOSYS        : 该类型微型打印机不支持此功能
 *
 * \note 可参考EPSON微打指令FS ( L Select label and black mark control function(s)
 */
 ...
 * \retval 无
 *
 * \note 驱动收到p_cmd命令后首先会检验命令的有效性，比如设备是否支持该指令，通过校验后会
 *       依次将buf1，buf2里面的数据写入微打设备，如果这条命令需要微打回应数据时p_rspbuf
 *       配置回应数据的缓冲区，rspbuf_len为回应数据的长度，若time_out时间内没有收到
 *       rspbuf_len长度的回应数据会产生超时，可通过p_cmd->ret_len判断收到回应的数据量，
 *       如果该命令不需要回应数据则p_rspbuf置为空。之所以指令数据分为buf1，buf2，是因为
 *       子功能码与打印数据不在一个连续的空间，分开方便操作，减少数据搬运操作。
 */
aw_local aw_inline

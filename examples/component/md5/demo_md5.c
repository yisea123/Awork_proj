...
void demo_md5_entry (void)
{
    ...
    aw_md5_init(&md5);                      /* 初始化md5算法 */
    aw_md5_update(&md5, str, strlen(str));  /* 更新填充输入值 */
    aw_md5_final(cal_result, &md5);         /* 计算结果 */

    if (0 == memcmp(std_result, cal_result, 16)) {
        AW_INFOF(("\nThe calculate is right!\n"
                  "The md5 value of \"abcdefghijklmnopqrstuvwxyz\" is ：\n"));
        for (i = 0; i < 16; i++) {
            AW_INFOF(("%x", cal_result[i]));
        }
    } else {

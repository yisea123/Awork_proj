 ...
 *
 * 本驱动为 ZM516X 模块的操作接口库，驱动的底层使用 UART 标准接口来操作 ZM516X 模块
 *
 * \internal
...
typedef struct awbl_zm516x_pwm {
     ...
     *        通道 3 的频率范围可设定从 62Hz~60000Hz。当 PWM 信号的频率为 0 时，表
     *        示不输出 PWM 信号
     */
    uint32_t freq[4];
 ...
 * \param[in]  dst_addr   如果需要获取远端模块配置信息，需要设置目标地址，否则忽略此参数
 ...
 * \retval -EPERM 修改失败
 *
 * \attention ZM516X 模块只有在串口数据位为 8 位，校验为无校验，停止位为 1 位时才能
 *            修改配置信息，在更改模块的数据位，校验位，停止位时需要注意，如果不小
 *            心改错了，需要把模块恢复出厂默认才能操作
 */
 ...
 * \retval -EPERM 设置失败
 */
aw_err_t awbl_zm516x_chan_set (int id, uint8_t chan, bool_t is_temp);
 ...
 * \retval -EPERM 设置失败
 */
aw_err_t awbl_zm516x_display_head_set (int id, bool_t is_show, bool_t is_temp);
 ...
 * \param[in] dir      管脚方向，需要使用或运算一次性传入所有管脚的配置，默认为输入，参见
 *                     AWBL_ZM516X_GPIO_INPUT* 或 AWBL_ZM516X_GPIO_OUTPUT*，
 *                     如 AWBL_ZM516X_GPIO_INPUT1 | AWBL_ZM516X_GPIO_OUTPUT2，将
 *                     配置 IO1、IO3、IO4 为输入，IO2 为输出
 * \param[in] is_temp  设置是否掉电丢失
 *
 * \retval  AW_OK 设置成功
 * \retval -EPERM 设置失败
 ...
 * \param[in] id       设备号
 * \param[in] dst_addr 要设置的 ZM516X 模块的网络地址
 * \param[in] value    管脚输出值，需要使用或运算一次性传入所有管脚的输出值，默认输出低电平，参见
 *                     AWBL_ZM516X_GPIO_LOW* 或 AWBL_ZM516X_GPIO_HIGH*，
 ...
 * \param[in]  id            设备号
 * \param[in]  dst_addr      要获取的 ZM516X 模块的网络地址
 * \param[out] p_trigger     获取到的 IO 触发边沿配置，如果不需要获取，传入 NULL 即
 *                           可。，参见 AWBL_ZM516X_EXTI_FALL* 或
 *                           AWBL_ZM516X_EXTI_RISE*
 * \param[out] p_period_10ms 获取到的上传周期配置，单位为 10ms，如果不需要获取，传
 *                           入 NULL 即可
 * \param[out] p_is_dormant  获取到的是否休眠配置，如果不需要获取，传入 NULL 即可。
 *                           指示模块是否处于休眠状态，TRUE 表示模块处于休眠状
 ...
 * \retval -EINVAL 无效参数
 */
aw_err_t awbl_zm516x_network_config (int id, uint8_t type, bool_t is_enable);
 ...
 *            来判断配置命令的结束，如果在调用了 awbl_zm516x_send 发送函数后，要紧

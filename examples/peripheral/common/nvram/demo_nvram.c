 ...
 *      awbl_hwconf_spi_flash0.h 中的存储段名称一致。
 *
 * - 实验现象:
 *   1. 串口打印操作结果信息。
 *
...
void demo_nvram_entry (void)
{
    ...
    if (aw_nvram_set("spi_flash0_nvram", 0, __g_buf, 0, BUF_SIZE) != AW_OK) {
    ...
    if (aw_nvram_get("spi_flash0_nvram", 0, __g_buf, 0, BUF_SIZE) != AW_OK) {
        AW_INFOF(("SPIFLASH NVRAM: Write Failed!\r\n"));
        return ;
    }

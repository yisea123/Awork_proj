...
/**
 * \file
 * \brief AW imx1050 eDMA 接口头文件
 *
 * 使用本模块需要包含以下头文件：
 * \code
 * #include "aw_imx1050_edma.h"
 * \endcode
 *
 * 更多内容待添加。。。
 *
...
#ifndef __AW_IMX1050_EDMA_H
#define __AW_IMX1050_EDMA_H
...
/** \brief DMA 驱动名 */
#define AWBL_IMX1050_EDMA_NAME                  "imx1050_edma"
...
enum dma_chan_cfg_status {
    CHAN_CFG_0 = 0,  /* 未配置状态  */
    CHAN_CFG_1 = 1,  /* 调用imx1050_edma_control(,DMA_SLAVE_CONFIG,,)进行了基础的配置 */
...
/**
 * \brief initialize AWBus imx1050 sdma service
 *
 * \attention the function should be called
 * after awbl_dev_init1() and before awbl_dev_init2()
 */
void awbl_imx1050_edma_init (void);

typedef void (*pfn_dma_callback_t)(void *param,unsigned int len);

void * imx1050_edma_alloc_chan(struct imx_dma_data *data);
int imx1050_edma_control(void *chan, enum dma_ctrl_cmd cmd,unsigned long arg);

int imx1050_edma_prep_memcpy(void *chan,
        dma_addr_t dma_dst,
        dma_addr_t dma_src,
        size_t len,
        unsigned long flags);

int imx1050_edma_prep_sg(void *chan,
 ...
 *
 */
int imx1050_edma_prep_memcpy_sg(void *chan,
        struct scatterlist *dst_sg,
        unsigned int dst_count,
        struct scatterlist *src_sg,
        unsigned int src_count,
...
int imx1050_edma_prep_slave_sg(void *chan,
        struct scatterlist *src_sg,
        unsigned int src_count,
        enum dma_transfer_direction direction);
 ...
 *
 */
int imx1050_edma_prep_dma_cyclic(void *chan,
        dma_addr_t dma_addr, size_t buf_len,
        size_t period_len, enum dma_transfer_direction direction);

int imx1050_edma_chan_start_sync(void *chan,uint32_t timeout);
int imx1050_edma_chan_start_async(void *chan,
        pfn_dma_callback_t callback,void *param);

int imx1050_edma_get_transfer_count(void *chan);
int imx1050_edma_chan_stop(void *chan);
int imx1050_edma_free_chan(void *chan);

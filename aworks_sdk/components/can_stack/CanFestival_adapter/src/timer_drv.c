...
typedef struct __timer_handle {
    TIMEVAL                  count;         /* 定时器计数 */
    TIMEVAL                  match_count;   /* 定时器匹配计数 */
    TIMEVAL                  now_count;     /* 当前计数 */
    TIMEVAL                  last_count;    /* 上一次调用getElapsedTime的计数 */
    AW_MUTEX_DECL           (cop_mutex);    /* 协议栈互斥锁 */
    aw_hwtimer_handle_t      p_hwtimer;     /* 硬件定时器句柄 */
    struct aw_isr_defer_job  isr_defer_job; /* 硬件定时器中断延迟作业 */
} __timer_handle_t;

...
/**
 * \file
 * \brief AWorks 动态任务的相关服务（任务创建、删除）例程
 *
 * - 操作步骤：
...
{
    aw_task2_id_t task2;                  /* 定义即将创建的任务2的ID，利用ID完成任务创建*/

    /* 
     * 可以在任务中创建另外一个任务，如下所示，创建任务2
     */
    task2 = aw_task2_create("task2",      /* 任务名字 */
                            2048,         /* 任务堆栈大小 */
                            4,            /* 任务优先级 */
                            task2_entry,  /* 任务入口函数 */
                            (void*)2);    /* 任务入口参数 */

    aw_task2_startup(task2);    /* 可以使用任务ID启动一个任务 */
...
void demo_task_dynamic_entry (void)
{
    ...
    /*
     * 常用的创建任务的方式是利用宏创建。
     * 使用宏操作创建任务两步曲: 1、动态创建任务 aw_task2_create
     *                         2、启动任务         aw_task2_startup
     */
    task1 = aw_task2_create("task1",      /* 任务名字 */
                            2048,         /* 任务堆栈大小 */
                            5,            /* 任务优先级 */
                            task1_entry,  /* 任务入口函数 */
                            (void*)1);    /* 任务入口参数 */

    aw_task2_startup(task1);    /* 启动task1,由于当前其优先级最高，进入运行函数task1_entry */
}

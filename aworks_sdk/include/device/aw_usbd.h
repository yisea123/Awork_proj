...
struct awbl_usbd_iad {
    ...
    uint8_t  if_count;
    uint8_t  fun_class;
    uint8_t  fun_subclass;
    uint8_t  fun_protocol;
    uint8_t  ifunc;
    bool_t   used;
};
...
struct aw_usbd_fun {
    ...
    uint8_t                        num_alt;     //只有设置0有效
    struct aw_usbd_fun            *p_next_alt;  //最后一个为NULL
    bool_t                         need_iad;
    uint8_t                       *p_spec_des;    //类自定义描述
    uint8_t                        spec_size;
    bool_t                         set;     /**< \brief 是否设置 */
...
struct aw_usbd_cfg {
    struct aw_usbd        *p_obj;   /**< \brief USBD设备指针 */
    struct awbl_usbd_iad   iads[AW_USBD_MAX_IAD_NUM];
 ...
 * \retval  FALSE        设备无效
 */
bool_t aw_usbd_valid (struct aw_usbd  *p_obj);
 ...
 * \retval  FALSE        接口功能无效
 */
bool_t aw_fun_valid (struct aw_usbd_fun  *p_fun);

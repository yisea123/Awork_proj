...
#define __FTPD_MAX_SESSIONS           FTPD_MAX_SESSIONS /* 同时支持的最多会话数  */
#define __FTPD_MAX_PATH_SIZE          256     /* 支持的路径最大长度      */
#define __FTPD_MAX_USER_NAME_LEN      32      /* 支持的最大用户名长度 */
#define __FTPD_CMD_BUF_SIZE           FTPD_CMD_BUF_SIZE    /* 运行缓存，应大于__FTPD_MAX_PATH_SIZE */
#if __FTPD_CMD_BUF_SIZE < __FTPD_MAX_PATH_SIZE
#error CFG ERROR: FTPD_CMD_BUF_SIZE too small
#endif

#define __FTPD_IDLE_TIMEOUT           60      /* idle timeout: 1 minutes */
#define __FTPD_RECV_TIMEOUT           60      /* recv timeout: 1 minutes */

#define __FTPD_TCP_KEEPIDLE           10      /* Idle time, 10 seconds */
#define __FTPD_TCP_KEEPINTVL          10      /* Probe interval, 10 seconds */
#define __FTPD_TCP_KEEPCNT            3       /* Probe count */
...
int aw_ftpd_init (const char *root_path,
                  int         listen_port,
                  int         anonymous,
                  bool_t    (*pfn_check_user) (const char *user_name,
                                               const char *pass_word))
{
    AW_INFOF(("ftpd: root_path=%s, listening_port=%d, anonymous=%s\n",
              root_path, listen_port, anonymous ? "yes" : "no"));

    aw_ftpd_param_init(&__g_ftpd, &__ftpd_cfg_info, root_path, listen_port, anonymous, pfn_check_user);

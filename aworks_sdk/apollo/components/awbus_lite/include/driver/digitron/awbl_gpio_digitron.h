...
struct awbl_gpio_digitron_param {
...
struct awbl_gpio_digitron_dev {
     ...
     * \brief 位码数据发送接口
     */
    void (*pfn_com_data_send)(struct awbl_gpio_digitron_dev *p_this, uint8_t idx);

    /**
     * \brief 段码数据发送接口
     */
    void (*pfn_seg_data_send)(struct awbl_gpio_digitron_dev *p_this, uint8_t idx);

    /**
     * \brief 用于管脚复用时接口
     */
    void (*pfn_black_cb)(struct awbl_gpio_digitron_dev *p_this, uint8_t idx);

};

aw_local aw_inline void awbl_gpio_digitron_com_data_send(struct awbl_gpio_digitron_dev *p_this, uint8_t idx)
{
	p_this->pfn_com_data_send(p_this, idx);
}

aw_local aw_inline void awbl_gpio_digitron_seg_data_send(struct awbl_gpio_digitron_dev *p_this, uint8_t idx)
{
    p_this->pfn_seg_data_send(p_this, idx);
}

aw_local aw_inline void awbl_gpio_digitron_black_cb(struct awbl_gpio_digitron_dev *p_this, uint8_t idx)
{

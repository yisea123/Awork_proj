...
/**
 * \addtogroup demo_if_edma_slave_sg
...
void demo_edma_slave_sg_entry(void){
    ...
    aw_serial_ioctl(2, SIO_BAUD_SET, (void *)UART_BAUD); // 设置波特率为9600bps
...
//    aw_cache_flush(rx_buf, len);
//    aw_cache_flush(tx_buf, len);
    ...
    tx_chan = imx1050_edma_alloc_chan(&data);
    if(tx_chan == NULL){
        AW_INFOF(("imx1050_edma_alloc_chan error\r\n"));
        return ;
    }
    ...
    ret = imx1050_edma_control(tx_chan, DMA_SLAVE_CONFIG, (unsigned long)&cfg);
    if(ret != AW_OK){
        AW_INFOF(("imx1050_edma_control error.\r\n"));
        return ;
    }
    ...
    ret = imx1050_edma_prep_slave_sg(tx_chan, &tx_list, 1, DMA_MEM_TO_DEV);
    if(ret != AW_OK){
        AW_INFOF(("imx1050_edma_prep_slave_sg error.\r\n"));
        return ;
    }
    ...
    rx_chan = imx1050_edma_alloc_chan(&data);
    if(rx_chan == NULL){
        AW_INFOF(("imx1050_edma_alloc_chan error.\r\n"));
        return ;
    }
    ...
    ret = imx1050_edma_control(rx_chan, DMA_SLAVE_CONFIG, (unsigned long)&cfg);
    if(ret != AW_OK){
        AW_INFOF(("imx1050_edma_control error\r\n"));
        return ;
    }
    ...
    rx_list.length = len;
    ret = imx1050_edma_prep_slave_sg(rx_chan, &rx_list, 1, DMA_DEV_TO_MEM);
    if(ret != AW_OK){
        AW_INFOF(("imx1050_edma_prep_slave_sg error.\r\n"));
        return ;
    }
    ...
    /* 开始传输 */
    ret = imx1050_edma_chan_start_async(rx_chan,
            dma_slave_sg_callback, (void *)rx_sem_id);
    if(ret != AW_OK){
        AW_INFOF(("imx1050_edma_chan_start_async error.\r\n"));
        return ;
    }


    /* 开始传输 */
    ret = imx1050_edma_chan_start_async(tx_chan,
    ...
    /* 释放发送通道   */
    ret = imx1050_edma_free_chan(tx_chan);
    if(ret != AW_OK){
        AW_INFOF(("imx1050_edma_free_chan error.\r\n"));
        return ;
    }

    /* 释放接收通道   */
    ret = imx1050_edma_free_chan(rx_chan);
    if(ret != AW_OK){
        AW_INFOF(("imx1050_edma_free_chan error.\r\n"));
        return ;
    }

//    aw_cache_invalidate(rx_buf, len);
//    aw_cache_invalidate(tx_buf, len);
    ret = memcmp(rx_buf, tx_buf, len);
    if(ret != 0){

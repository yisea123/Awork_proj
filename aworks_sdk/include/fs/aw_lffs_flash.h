...
#define AW_LFFS_FLASH_NO_ERR       0       //!< no error
#define AW_LFFS_FLASH_ECC_OK       1       //!< bit-flip found, but corrected by ECC
#define AW_LFFS_FLASH_NOT_SEALED   2       //!< page spare area is not sealed properly (only for ReadPageWithLayout())
#define AW_LFFS_FLASH_IO_ERR       -1      //!< I/O error
#define AW_LFFS_FLASH_ECC_FAIL     -2      //!< ECC failed
#define AW_LFFS_FLASH_BAD_BLK      -3      //!< bad block
#define AW_LFFS_FLASH_CRC_ERR      -4      //!< CRC failed
#define AW_LFFS_FLASH_UNKNOWN_ERR  -100    //!< unkown error?

#define AW_LFFS_FLASH_HAVE_ERR(e)      ((e) < 0)

#if defined(AW_LFFS_CONFIG_BAD_BLOCK_POLICY_STRICT)
# define AW_LFFS_FLASH_IS_BAD_BLOCK(e) \
    ((e) == AW_LFFS_FLASH_ECC_FAIL || (e) == AW_LFFS_FLASH_ECC_OK || (e) == AW_LFFS_FLASH_BAD_BLK || (e) == AW_LFFS_FLASH_CRC_ERR)
#else
# define AW_LFFS_FLASH_IS_BAD_BLOCK(e) \
    ((e) == AW_LFFS_FLASH_ECC_FAIL || (e) == AW_LFFS_FLASH_BAD_BLK || (e) == AW_LFFS_FLASH_CRC_ERR)
#endif
...
typedef struct aw_lffs_flash_ops {
     ...
     *   if ecc_opt is AW_LFFS_ECC_HW, flash driver must calculate and return ecc of data(if ecc != NULL).
     *   if ecc_opt is AW_LFFS_ECC_HW_AUTO, flash driver do ecc correction before return data and flash driver do not need to return ecc.
     *   if ecc_opt is AW_LFFS_ECC_NONE or AW_LFFS_ECC_SOFT, flash driver do not need calculate data ecc and return ecc.
     *
     * \param[out] ecc_store ecc store on spare area
     *   if ecc_opt is AW_LFFS_ECC_NONE or AW_LFFS_ECC_HW_AUTO, do not need to return ecc_store.
     *
     * \note flash driver must provide this function if layout_opt is AW_LFFS_LAYOUT_FLASH.
     ...
     *          #AW_LFFS_FLASH_ECC_FAIL: page data has flip bits and ecc correct failed (when ecc_opt == AW_LFFS_ECC_HW_AUTO)
     *          #AW_LFFS_FLASH_ECC_OK: page data has flip bits and corrected by ecc (when ecc_opt == AW_LFFS_ECC_HW_AUTO)
     *          #AW_LFFS_FLASH_BAD_BLK: if the block is a bad block (e.g., the bad block mark byte is not 0xFF)
     ...
     *       then read bad block mark and return AW_LFFS_FLASH_BAD_BLK if bad block mark is not 0xFF.
     *
     ...
     *
     * \note if layout_opt is AW_LFFS_LAYOUT_FLASH or ecc_opt is AW_LFFS_ECC_HW/AW_LFFS_ECC_HW_AUTO, flash driver MUST implement
     *       this function. LFFS will use this function (if provided) prio to 'WritePage()'
     *
     * \param[in] ecc ecc of data. if ecc_opt is AW_LFFS_ECC_SOFT and this function is implemented,
     *                LFFS will calculate page data ecc and pass it to WritePageWithLayout().
     *            if ecc_opt is AW_LFFS_ECC_NONE/AW_LFFS_ECC_HW/AW_LFFS_ECC_HW_AUTO, LFFS pass ecc = NULL.
     *
     ...
     *
     * \note if this function is not implented by driver, LFFS check the block_status byte in spare.
     *
     ...
     *
     * \note if this function is not implemented, LFFS mark bad block by call 'WritePage()/WritePageWithLayout()'
     ...
     *
     * \note if this function is not implemented, LFFS will read all pages and verify that all bits are set to 1'
     *
...
int aw_lffs_flash_page_tag_read (aw_lffs_device_t *dev, int block, int page, aw_lffs_tags_t *tag);

/** read page data to page buf and do ECC correct */
int aw_lffs_flash_page_read (aw_lffs_device_t *dev, int block, int page, aw_lffs_buf_t *buf, bool_t skip_ecc);

/** write page data and spare */
int aw_lffs_flash_page_combine_write (aw_lffs_device_t *dev, int block, int page, aw_lffs_buf_t *buf, aw_lffs_tags_t *tag);

/** Mark this block as bad block */
int aw_lffs_flash_bad_block_mark (aw_lffs_device_t *dev, int block);

/** Is this block a bad block ? */
bool_t aw_lffs_flash_is_bad_block (aw_lffs_device_t *dev, int block);
...
/** load lffs_FileInfo from flash storage */
aw_err_t aw_lffs_flash_file_info_read (aw_lffs_device_t *dev, int block, int page, aw_lffs_file_info_t *info);
/**
 * Initialize LFFS flash interface
 */
aw_err_t aw_lffs_flash_interface_init (aw_lffs_device_t *dev);

/**
 * Release LFFS flash interface
 */

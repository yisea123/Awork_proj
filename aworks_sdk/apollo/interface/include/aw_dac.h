 ...
 *
 *  aw_dac_val_set(0, dac_val);               // 设置DAC通道0的输出电压为 1.2V (未使能，还未实际输出)
 *  aw_dac_enable(0);                         // 使能输出，输出电压为1.2V
 *
 *  dac_val = 1400 * (1 << dac_bits) / dac_vref; 
 *  aw_dac_val_set(0, dac_val);               // 设置DAC通道0的输出电压为 1.4V (前面已使能，实际输出电压1.4V)
 *
 * \endcode
 *
 * \internal
 ...
 *       如果先调用该使能函数，则DAC使能后输出电压为0，可以通过后续调用aw_dac_val_set()设置为需要的值。
 *       如果先调用了用aw_dac_val_set()函数设置了电压值，则使能后即为相应的电压输出值。
 */

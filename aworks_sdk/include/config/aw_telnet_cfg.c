...
aw_err_t aw_telnet_shell_cfg()
{
	...
	_init_task(&__telnet_srv.tsk_cont, &srv_task.tcb, "telnet srv task",
			AW_CFG_TELNET_TASK_PRIO, srv_task.stack, AW_CFG_TELNET_TASK_STACK_SIZE,
			aw_telnet_srv_run, &__telnet_srv);
	for (i = 0; i < AW_CFG_TELNET_MAX_SESSION; i++) {
		_init_task(&__telnet_session[i].tsk_cont, &session_task[i].tcb, "telnet session task",
				AW_CFG_TELNET_TASK_PRIO, session_task[i].stack, AW_CFG_TELNET_TASK_STACK_SIZE,
				aw_telnet_session_run, &__telnet_session[i]);
		_init_task(&__shell_dev[i].tsk_cont, &ts_task[i].tcb, "telnet shell task",
				AW_CFG_TELNET_TASK_PRIO, ts_task[i].stack, AW_CFG_TELNET_TASK_STACK_SIZE,
				aw_telnet_shell_run, &__shell_dev[i]);

		shell_opt.p_his_buf     = __g_his_mem[i];
		__g_tel_shell_opt[i]    = shell_opt;
	}

	return aw_telnet_shell_init(&__telnet_srv, __telnet_session,
			__shell_dev, AW_CFG_TELNET_MAX_SESSION, AW_CFG_TELNET_LISTEN_PORT,
			AW_CFG_TELNET_USER_NAME, AW_CFG_TELNET_USER_PSW, __g_tel_shell_opt);
}

 ...
 * step3: CANopen process task start => aw_cop_mst_process
 * step4: CANopen stack start        => aw_cop_mst_start
 * step5: PDO register               => aw_cop_mst_input_pdo_install
 *
 * \internal
 ...
 *
 * \retval  FALSE: 接收到的数据不保存到队列里面，
 *                 则通过aw_cop_mst_input_pdo_get()不能获取到接收到的数据
 * \retval  TRUE:  接收到的数据保存到队列里面，
 *                 则通过aw_cop_mst_input_pdo_get()可以获取到接收到的数据
 *
 */
typedef bool_t (*aw_cop_mst_pfn_pdo_recv_t) (aw_cop_pdo_msg_t *p_msg);
 ...
 * \param[out] p_length:  接收数据大小，返回实际接收到的数据

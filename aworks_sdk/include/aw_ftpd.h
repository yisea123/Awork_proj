...
typedef struct aw_ftpd_session {
    ...
    int                 sock_data;
    time_t              time_start;
    char               *root_path; /* user root path */
    char               *path_end;  /* user path ending position */
...
typedef struct aw_ftpd {
	...
	int                  max_sessions;
    bool_t             (*check_user) (const char *name, const char *passwd);
    const char          *root_path;  /** \brief root path of ftp server */
    bool_t               anonymous;  /** \brief anonymous user enabled or not */
...
void aw_ftpd_param_init (aw_ftpd_t              *ftpd,

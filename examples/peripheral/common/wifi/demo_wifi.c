 ...
 *
 * - 实验现象：
 *   01. 执行 "scan" 可以查看附近的可见热点
 *   02. 执行 "scan passive" 可以查看附近的所有热点
 *   03. 执行 "join ZYWIFI meiyoumima" 加入名称为 ZYWIFI, 密码为 meiyoumima 的热点
 *   04. 执行 "getapinfo" 获取已加入的热点的信息（信号强度等）
 *   05. 执行 "leave" 离开已加入的热点
 *   06. 执行 "startap AWWIFI wpa2_aes meiyoumima 6" 在 6 号 频道创建名称为 AWWIFI, 密码为 meiyoumima 的热点
 *   07. 执行 "maxassoc" 获取最多可加入本地创建的热点的客户端的数量
 *   08. 执行 "getstalist" 获取已加入本地创建的热点的客户端信息
 *   09. 执行 "stopap" 关闭已创建的的热点
 *   10. 执行 "getlinkstatus" 获取 Wi-Fi 链路状态
 *
 * \par 源代码
...
#include "net/aw_dns.h"
#include "net/aw_hostname.h"
...
aw_local bool_t g_sta_is_assocated = FALSE;
...
aw_local int __shell_join (int argc, char* argv[], struct aw_shell_user *sh)
{
    ...
    do {
        ssid.length = snprintf((char *) ssid.value, sizeof(ssid.value), argv[0]);
        pkey.length = snprintf((char *) pkey.value, sizeof(pkey.value), argv[1]);
        ...
        if (argc > 2) {
            ...
            while (FALSE == bound && dhcp_wait_time) {
                aw_mdelay(100);
                dhcp_wait_time -= 100;
...
aw_local const struct aw_shell_cmd __g_shell_cmd_tbl[] = {
    {
        __shell_scan,
        "scan",
        "<passive> Scan all enabled channels and print a list of APs found." ENDL
        "\tp : Passively scan a network by listening for beacons from APs"
    },
    {
        __shell_join,
        "join",
        "<ssid> <key> [ip] [netmask gateway] [dns]" ENDL
    ...
    },
    {
        __shell_get_ap_info,
        "getapinfo",
        "<ap> Get specify AP config. Non arg for joined AP"
    },
    {
        __shell_start_ap,
        "startap",
        "Bring up local SoftAP" ENDL
        "<ssid> <open|wpa2|wpa2_aes> <key> <channel> [ip netmask gateway]"
    },
    {
        __shell_stop_ap,
        "stopap",
        "Shutdown local SoftAP"
    },
    {
        __shell_get_sta_list,
        "getstalist",
    ...
    },
    {
        __shell_link_status,
        "getlinkstatus",
        "Get link status of the Wi-Fi interface"
    },
    {
        __shell_set_tx_pwr,
        "settxpwr",
        "<dbm> Set the tx power"
    },
    {
        __shell_get_tx_pwr,
        "gettxpwr",
    ...
    },
    {
        __shell_set_country,
        "setcc",
        "<country> Set the country code, cn/us..."
    },
    {
        __shell_get_country,
        "getcc",
    ...
    },
    {
        __shell_11n,
        "set11n",
        "[1|0] Enable or disable 11n support"
    },
    {
        __shell_powersave,
        "powersave",
        "[1|0] Enables powersave mode"
    },
    {
        __shell_get_max_assoc,
        "maxassoc",
        "Get the maximum number of associations supported by all interfaces (STA and Soft AP)" ENDL
        "\tIf the STA interface is associated then the Soft AP can support" ENDL
        "\t(max_assoc - 1) associated clients."
    },
    {
        __shell_get_rate,
        "rate",
        "<0|1> STA Interface or AP Interface" ENDL
        "\tGet the current data rate for the provided interface (STA and Soft AP)"
    },
    {
        __shell_get_channel,
        "getch",
        "<0|1> STA Interface or AP Interface" ENDL
        "\tGet the current channel on the WLAN radio (STA and Soft AP)" ENDL
        "\tOn most WLAN devices this will get the channel for both AP *AND* STA"
    },
    {
        __shell_set_channel,
        "setch",
        "[ch] <0|1> STA Interface or AP Interface" ENDL
        "\tSet the current channel on the WLAN radio (STA and Soft AP)" ENDL
        "\tOn most WLAN devices this will set the channel for both AP *AND* STA"
    },
    {
        __shell_monitor_mode,
        "setmm",
        "[1|0] Enable or disable monitor mode"
    },
    {
        __shell_fbt_over_ds,
        "fbt",
    ...
    },
    {
        __shell_driver_log,
        "drvlog",
        "<1|0> Enable or disable driver log"
    },
};

/******************************************************************************/
aw_local void aw_wifi_shell_init (void)
{

...
struct awbl_adc_servfuncs {
     ...
     * \param[in] p_desc       缓冲区描述,可以是单个（单个缓冲区描述的地址）或多个（缓冲区描述数组的地址）
     * \param[in] desc_num     指定缓冲区描述的个数
     * \param[in] count        序列的转换次数（一个转换序列表示desc_num指定个数的的缓冲区依次填充满），
     *                         如果指定为0，则会持续不断的转换，即一个序列结束，自动继续进行下一个序列
     * \param[in] pfn_seq_complete  序列完成回调函数,当一个序列转换完成时：
     *                               - 单个缓冲区，缓冲区转换完成
     *                               - 多个缓冲区，从第一个缓冲区到最后一个缓冲区依次填充满
     *                                 应该调用该函数设置的回调函数。当一个序列异常结束时，
     *                                 也应调用该函数设置的回调函数，并指明错误信息

 ...
 *
 * - 实验现象：
 *   1. 输入shell命令："playback start"，播放音频；
 *   2. 输入shell命令："playback pause"，暂停音频；
 *   3. 输入shell命令："playback stop"，停止播放。
 *
 * \par 源代码
                                ...
                                struct aw_shell_user *sh)
{
    ...
    /* 发送消息 */
    AW_MSGQ_SEND(wav_msgq, &play_cmd, sizeof(play_cmd_t),
                 AW_MSGQ_WAIT_FOREVER, AW_MSGQ_PRI_NORMAL);

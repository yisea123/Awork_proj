...
#define AWBL_ZLG600A_DEV_IOCTL_BAUD_SET           __AW_IOW(AWBL_ZLG600A_DEV_IOCTL_BASE, 1, int)

/** \brief get current card status */
#define AWBL_ZLG600A_DEV_IOCTL_ADDR_SET           __AW_IOW(AWBL_ZLG600A_DEV_IOCTL_BASE, 2, int)

/** \brief get current card status */
#define AWBL_ZLG600A_DEV_IOCTL_FRAME_FMT_SET      __AW_IOW(AWBL_ZLG600A_DEV_IOCTL_BASE, 3, int)
...
struct awbl_zlg600a_servopts {
    ...
    /** \brief 帧发送函数         */
    aw_err_t (*pfn_frame_send) (void *p_cookie, awbl_zlg600a_frame_base_t *p_frame);

    /** \brief 帧接收函数         */
    aw_err_t (*pfn_frame_recv) (void *p_cookie, awbl_zlg600a_frame_base_t *p_frame);
    /** \brief 设备控制类函数 */
 ...
 * \retval -AW_EIO    : 设置失败，数据通信出错
 *
 * \note 在串口通信方式时设置了新旧帧格式后将以原来的帧格式返回成功信息，但在I2C 通信方式时，
 *       收到“设置新旧帧格式”命令后，即默认进入了设置后帧格式，不返回成功信息，下一次与模块通信需要
 *       使用设置后的帧格式与模块通信。同时为了保证I2C通信方式下，帧格式设置成功，下一次与模块通信
 *       需要延时至少125ms
 */
...
#define AWBL_ZLG600A_MIFARE_CARD_REQ_IDLE 0x26 /**< \brief IDLE模式,请求空闲的卡 */
#define AWBL_ZLG600A_MIFARE_CARD_REQ_ALL  0x52 /**< \brief ALL模式,请求所有的卡  */
 ...
 * \retval -AW_EIO    : 读取失败，数据通信出错
 */
aw_err_t awbl_zlg600a_mifare_card_val_get (int id, uint8_t nblock,int32_t *p_value);
 ...
 * 信息来选择awbl_zlg600a_cicc_tpdu()函数使用的传输协议（T = 0 或T = 1）。该函数没
 * 有自动执行PPS命令，需要用户根据复位信息来判断是否使用awbl_zlg600a_cicc_pps()来
 * 修改协议和参数。
 *
 * \param[in] id            : 设备ID号

...
struct aw_shell_context {
    ...
    bool_t                   des_exist;         /**< \brief 描述符存在的标志 */
    ...
    bool_t                   is_cmdset;         /**< \brief 是否是在命令集 */
#endif /* AW_SHELL_USE_CMDSET */

    int                      opt_start;         /**< \brief opt的下标 */
    int                      opt_end;           /**< \brief 下一个opt的下标 */
    bool_t                   opt_is_valid;      /**< \brief 当前选项是否有效 */

    bool_t                   inited;            /**< \brief 初始化标志 */
    bool_t                   color_enable;      /**< \brief 颜色使能 */
...
#define __CHART_ITEM_MAX   15
    ...
    bool_t                is_default_hframe;       /**< \brief 是否使用默认边框  */
    bool_t                wait_use_hframe;     /**< \brief 等待使用默认边框  */
...
/**
 * \brief 添加一个固定最大长度的列项，该项下的内容的长度超过该项的最大长度时会自动换行
 *
 * \param[in]       p_this      aw_shell_chart结构体地址
 ...
 * \retval AW_OK    运行成功
 */
aw_err_t aw_shell_init(struct aw_shell_context *sc, struct aw_shell_user *sh, aw_shell_opt_t *p_opt);

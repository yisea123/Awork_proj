...
/**
 * \addtogroup demo_if_i2c_rtc_async
...
#define I2C_SLAVE_ADDR        0X51           /*从机地址(用户可查看具体I2C丛机的配置文件，可根据根据配置文件修改丛机地址) */
#define I2C_MASTER_BUSID      1              /*从机使用I2C1(用户可查看具体I2C丛机的配置文件，根据根据配置文件修改I2C总线) */

#define SLAVE_REG_SEC         0x02           /*器件内子地址，从此地址开始写入数据(用户可根据具体的I2C丛机设备修改)*/
...
static void __rtc_callback(void *p_arg){
    ...
    AW_SEMB_GIVE(sem_sync);          //释放信号量
}

/**
 * \brief I2C异步写数据
 */
static bool_t __aw_i2c_async_write (aw_i2c_device_t *i2c_slave, uint8_t *data,size_t size){
    ...
    // 等待消息处理完成
    ret = aw_semb_take(&sem_sync, AW_SEM_WAIT_FOREVER);
    if (ret != AW_OK) {
        return ret;
    }
 ...
 * \brief I2C异步读数据
 */
static bool_t __aw_i2c_async_read (aw_i2c_device_t *i2c_slave, uint8_t *data,size_t size){
    ...
    aw_i2c_mktrans(&trans[1],
                   I2C_SLAVE_ADDR,                 // 目标器件地址 (将被忽略)
                   AW_I2C_M_RD | AW_I2C_M_NOSTART, // 写操作, 不发送起始信号及从机地址
                   &data[0],                       // 数据缓冲区
                   size);                             // 数据个数

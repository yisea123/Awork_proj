...
#include "net/aw_netif_adapter.h"
#include "net/aw_net_buf.h"

#ifdef AW_NETIF_EN_PCAP
#include "net/aw_pcap.h"
#endif
...
typedef struct aw_netif_info_get_entry {
    ...
    aw_err_t  (*get_hwaddr)      (uint8_t *p_hwaddr, int addr_len); /** \brief 指定网卡的MAC地址 */
    bool_t    (*get_dhcp_en)     (void); /** \brief 默认启动DHCP获取地址（需要开启DHCP客户端功能） */
...
#else
    aw_pfuncvoid_t pfn_pad0; /* 避免 size 改变 */
#endif
};
 ...
 * \retval -AW_ENODEV  设备不存在
 */
aw_err_t aw_netif_is_up (aw_netif_t *p_netif, bool_t *p_up);
 ...
 * \retval -AW_ENODEV  设备不存在
 */
aw_err_t aw_netif_is_link_up (aw_netif_t *p_netif, bool_t *p_up);
 ...
 * \return 设备存在与否
 */
aw_static_inline bool_t aw_netif_is_exist (aw_netif_t *p_netif)
{
 ...
 * \return 设备就绪与否
 */
aw_static_inline bool_t aw_netif_is_ready (aw_netif_t *p_netif)
{
    return (p_netif && (p_netif->state & (AW_NETIF_STAT_EXIST | AW_NETIF_STAT_DEV_UP)));
}
 ...
 * \retval -AW_ENODEV  设备不存在
 */
aw_err_t aw_netif_hwaddr_get_by_name (const char *p_name, uint8_t *p_addr, uint8_t len);
...
aw_err_t aw_netif_dhcpc_is_on (aw_netif_t *p_netif, bool_t *on);

aw_err_t aw_netif_dhcpc_is_bound (aw_netif_t *p_netif, bool_t *bound);

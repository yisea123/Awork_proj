...
/** \brief 第一阶段，数据块正在从旧块写入数据，完成后写入AW_FTL_BLOCK_TAG_DATA_IN_LOG */
#define AW_FTL_BLOCK_TAG_DATA_IN_BLK  0x5F

/** \brief 第二阶段，数据块正在从日志块写入数据，完成后写入AW_FTL_BLOCK_TAG_DATA */
...
bool_t aw_ftl_logic_blk_is_empty (struct aw_ftl_dev *p_ftl, uint16_t lbn);

bool_t aw_ftl_logic_sector_be_used (struct aw_ftl_dev *p_ftl,
                                    uint16_t           lbn,
                                    uint8_t            sector);

bool_t aw_ftl_logic_sector_is_dirty (struct aw_ftl_dev *p_ftl,
                                     uint16_t           lbn,
                                     uint8_t            sector);

bool_t aw_ftl_log_sector_is_dirty (struct aw_ftl_dev *p_ftl,
                                   uint16_t           log,
                                   uint8_t            sector);

...
typedef struct awbl_imx1050_gpt_devinfo {
    ...
    uint32_t           mode;           /**< \brief 模式(定时器、输入捕获、输出比较) */
    uint32_t           feature;        /**< \brief 输入输出特性 */
    uint32_t           chan;           /**< \brief 输入输出通道 */
    uint32_t           output_compare; /**< \brief 输出比较值 */
...
typedef struct awbl_imx1050_gpt_dev {
    ...
    imx1050_gpt_regs_t     *p_reg;                 /**< \brief 寄存器基地址 */
...
/**
 * \brief 将 IMX1050 GPT 驱动注册到 AWBus 子系统中
 *

...
void dhcps_set_addr_pool (struct dhcp_server *p_ctx, uint32_t start, size_t limit);
aw_err_t dhcps_set_addr_pool_def (struct dhcp_server *p_ctx);

#ifdef DHCPS_CLIENT_NAME_EN
char *dhcps_get_host_name_by_macaddr (struct dhcp_server *p_ctx, aw_eth_mac_t *mac);
char *dhcps_get_host_name_by_ipaddr (struct dhcp_server *p_ctx, struct in_addr *ip);
#endif
aw_err_t dhcps_get_ip_by_macaddr (struct dhcp_server *p_ctx, aw_eth_mac_t *mac, struct in_addr *ip);
...
bool_t dhcps_is_on (struct dhcp_server *p_ctx);

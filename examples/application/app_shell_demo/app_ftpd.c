 ...
 *      - AW_COM_FTPD
 *      - 需要挂载的存储设备如SD卡
 *   2. 以SD卡为例。在shell界面进入".test"子目录后输入"mount /sd0 /dev/sd0 vfat"挂载文件系统
 *   3. 使用网线与PC相连，确保二者在同一网段下
 *   4. 输入"ftpd / 21"或者"ftpd / 21 AWorks 1324"
 *   5. 在PC的文件管理器中输入"ftp://开发板IP"，如"ftp://192.168.12.110"
 *   6. 若第4步中输入的是"ftpd / 21"则可以直接在PC中访问开发板中SD卡的内容，若第4步中输入的是

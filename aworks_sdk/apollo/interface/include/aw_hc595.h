 ...
 *
 *        该类用于描述一块74HC595芯片, 属于抽象类. 根据不同的驱动方式, 由子类实现对其的
 *    操作, 例如, 使用GPIO驱动74HC595, 使用SPI驱动74HC595.
 *
 *  74HC595引脚及其定义
 *
 *               -------------
...
typedef struct aw_hc595_dev {
	 ...
	 * \brief hc595器件移位操作接口
	 */
	void (*pfn_hc595_shift) (struct aw_hc595_dev *p_this, uint8_t bit_order, uint8_t data);
 ...
 * \param  bit        待移位数据.
 */
aw_local aw_inline void aw_hc595_shift (aw_hc595_dev_t *p_this, uint8_t bit_order, uint8_t data)
{

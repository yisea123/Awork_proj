...
/** \brief 申明系统全局defer对象，具体定义分别在aw_isr_defer_cfg.c和aw_nor_defer_cfg.c */
aw_export struct aw_defer g_sys_isr_defer;  /**< \brief 声明全局紧急类型defer对象 */
aw_export struct aw_defer g_sys_nor_defer;  /**< \brief 声明全局普通类型defer对象 */
 ...
 *  {
 *      if (aw_sys_defer_job_is_usable(AW_SYS_DEFER_TYPE_ISR, &g_myjob) == TRUE) {
 *          // 节点可用，将其添加到队列中
 ...
 * \endcode
 */
aw_static_inline bool_t aw_sys_defer_job_is_usable (aw_sys_defer_t       defer_t,
...
/**
 * \brief 将一个工作节点添加至系统延迟作业中处理，节点添加后只能被执行一次（即会被自动清除）,
 *        若当前工作节点已添加，那直接返回错误
 *
 * \param[in] defer_t  系统延迟作业类型

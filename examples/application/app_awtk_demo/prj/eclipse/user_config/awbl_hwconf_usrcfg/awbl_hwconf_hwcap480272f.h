...
#ifndef __AWBL_HWCONF_HWCAP480272F_H
#define __AWBL_HWCONF_HWCAP480272F_H

#ifdef AW_DEV_HWCAP480272F
...
#define __HW480272_DCLK_F       9000000

#define DOTCLK_H_ACTIVE         __HW480272_X_RES
#define DOTCLK_H_PULSE_WIDTH     41//1
#define DOTCLK_HF_PORCH          4//8
#define DOTCLK_HB_PORCH          8 //15
...
#define DOTCLK_V_PULSE_WIDTH     10//1
#define DOTCLK_VF_PORCH          4//4
#define DOTCLK_VB_PORCH          2//12
#define DOTCLK_V_WAIT_CNT       (DOTCLK_V_PULSE_WIDTH + DOTCLK_VB_PORCH)
#define DOTCLK_V_PERIOD         (DOTCLK_VF_PORCH + DOTCLK_V_ACTIVE + DOTCLK_V_WAIT_CNT)


typedef struct __hwcap480272f_gpio_cfg {
    int         gpio;
    uint32_t    func;

} __hwcap480272f_gpio_cfg_t;


aw_local aw_const  __hwcap480272f_gpio_cfg_t __g_hwcap480272f_gpios_list[] = {

    {GPIO3_0,  AW_GPIO_OUTPUT_INIT_LOW_VAL},  /* lcd复位 */
    {GPIO1_10, AW_GPIO_OUTPUT_INIT_LOW_VAL},  /* 背光控制,当使用PWM时将这句注释掉 */
...
aw_local void __hwcap480272_plfm_init (void)
{
    int i = 0;
    int flexspi_gpios[AW_NELEMENTS(__g_hwcap480272f_gpios_list)];

    for (i = 0; i < AW_NELEMENTS(__g_hwcap480272f_gpios_list); i++) {
        flexspi_gpios[i] = __g_hwcap480272f_gpios_list[i].gpio;
    }
    ...
    if (aw_gpio_pin_request("lcd_gpios",
        ...
        aw_gpio_pin_cfg(__g_hwcap480272f_gpios_list[1].gpio,
                          __g_hwcap480272f_gpios_list[1].func);

        /* 复位lcd */
        aw_gpio_pin_cfg(__g_hwcap480272f_gpios_list[0].gpio,
                        __g_hwcap480272f_gpios_list[0].func);

        aw_gpio_set(__g_hwcap480272f_gpios_list[0].gpio, 0);

        aw_udelay(200);

        aw_gpio_set(__g_hwcap480272f_gpios_list[0].gpio, 1);

        aw_udelay(50);

        for (i = 2; i < AW_NELEMENTS(__g_hwcap480272f_gpios_list); i++) {
            /* 配置功能引脚 */
            aw_gpio_pin_cfg(__g_hwcap480272f_gpios_list[i].gpio,
                            __g_hwcap480272f_gpios_list[i].func);
        }
    }
}

/* 设备信息 */
aw_local aw_const awbl_imx1050_tft_panel_devinfo_t  __g_hwcap480272f_param = {
    /* panel_info */
    {
        ...
        100,                             /* default back light intensity */
    },
    /* timing */
    {
    ...
    /* plfm_init */
    __hwcap480272_plfm_init
};

/* 设备实例内存静态分配 */
aw_local awbl_imx1050_tft_panel_t __g_hwcap480272f;

#define AWBL_HWCONF_HWCAP480272F    \
    {                               \
        AWBL_IMX1050_TFT_PANEL_NAME,\
        0,                          \
        AWBL_BUSID_PLB,             \
        0,                          \
        &(__g_hwcap480272f.dev),       \
        &(__g_hwcap480272f_param)      \
    },

#else
#define AWBL_HWCONF_HWCAP480272F

#endif  /* AW_DEV_HWCAP480272F */

#endif  /* __AWBL_HWCONF_HWCAP480272F_H */

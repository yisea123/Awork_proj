...
#define  NAND_FLASH_DEV_NAME  "awbl_imx1050_nandflash_drv_name"     /*nandflash的驱动名称根据具体硬件平台修改*/
...
void demo_nand_entry (void)
{
    ...
    p_chip = __nand_dev_find(NAND_FLASH_DEV_NAME, i);

    /* 如果块i是坏块，继续循环 */
    while (awbl_nand_blk_is_bad(p_chip, i) == TRUE) {
        i++;
    }
    ...
    if(ret != AW_OK){
        aw_kprintf("awbl_nand_blk_erase fail!\r\n");
        return ;
    }else{

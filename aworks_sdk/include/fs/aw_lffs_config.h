...
/**
 * \brief 使能保存文件信息到表中，可以加快mount的速度，每个块需要保存12字节的数据，因此
 *        设置保存时间  AW_LFFS_CONFIG_INFO_TABLE_DELAY_MS 应尽可能大
 */
 ...
 *
 * \note AW_LFFS_CONFIG_BAD_BLOCK_POLICY_STRICT should be disabled if this config is
 *       choosen.
 */
...
#define AW_LFFS_SPARE_BUFFER_SIZE      (AW_LFFS_MAX_SPARE_BUFFERS * AW_LFFS_MAX_SPARE_SIZE)
...
#if (AW_LFFS_MAX_PAGE_BUFFERS - AW_LFFS_CLONE_BUFFERS_THRESHOLD - 1 < AW_LFFS_MAX_DIRTY_PAGES_IN_A_BLOCK)
#error "AW_LFFS_MAX_DIRTY_PAGES_IN_A_BLOCK should < (AW_LFFS_MAX_PAGE_BUFFERS - AW_LFFS_CLONE_BUFFERS_THRESHOLD)"
#endif

#if defined(AW_LFFS_CONFIG_PAGE_WRITE_VERIFY) && (AW_LFFS_CLONE_BUFFERS_THRESHOLD < 2)
#error "AW_LFFS_CLONE_BUFFERS_THRESHOLD should >= 2 when AW_LFFS_CONFIG_PAGE_WRITE_VERIFY is enabled."
#endif

#if AW_LFFS_CONFIG_USE_STATIC_MEMORY_ALLOCATOR + AW_LFFS_CONFIG_USE_SYSTEM_MEMORY_ALLOCATOR > 1
#error "Please enable ONLY one memory allocator"
#endif

#if AW_LFFS_CONFIG_USE_STATIC_MEMORY_ALLOCATOR + AW_LFFS_CONFIG_USE_SYSTEM_MEMORY_ALLOCATOR == 0
#error "Please enable ONE of memory allocators"
#endif

#if defined(AW_LFFS_CONFIG_USE_GLOBAL_FS_LOCK) && defined(AW_LFFS_CONFIG_USE_PER_DEVICE_LOCK)
...
#if defined(AW_LFFS_CONFIG_BAD_BLOCK_POLICY_STRICT) && defined(AW_LFFS_CONFIG_LFFS_REFRESH_BLOCK)

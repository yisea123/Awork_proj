...
#define AWBL_UPP_FT_IN          1   /**< （驱动开发者使用）告诉标准层，驱动支持输入特性  */
#define AWBL_UPP_FT_OUT         2   /**< （驱动开发者使用）告诉标准层，驱动支持输出特性  */
#define AWBL_UPP_FT_BUILDIN_DMA 8   /**< （驱动开发者使用）告诉标准层，驱动支持内部DMA特性  */
    uint16_t    features;            /**< 控制器的特性(支持的传输控制标志)  */

    uint16_t    align_size;         /**< 设备对齐要求 */
    uint32_t    max_xfer_size;      /**< 设备每次启动最大传输量 */

#define AWBL_UPP_CMD_OPEN       1   /**< \brief （驱动开发者使用）告诉驱动打开设备 */
#define AWBL_UPP_CMD_XFER_START 2   /**< \brief （驱动/应用开发者使用）告诉驱动开启传输 */
#define AWBL_UPP_CMD_XFER_STOP  3   /**< \brief （驱动/应用开发者使用）告诉驱动开启传输*/
#define AWBL_UPP_CMD_CLOSE      4   /**< \brief （驱动开发者使用）告诉驱动关闭设备 */
#define AWBL_UPP_CMD_SET_OUTPUT 5   /**< \brief （驱动/应用开发者使用）告诉驱动设置为输出 */
#define AWBL_UPP_CMD_SET_INPUT  6   /**< \brief （驱动/应用开发者使用）告诉驱动设置输入 */
#define AWBL_UPP_CMD_SET_MSGQID 7   /**< \brief （驱动开发者使用）驱动设置一个初始化好的msgq给标准层使用 */
#define AWBL_UPP_CMD_RESET      8   /**< \brief （驱动/应用开发者使用）重置 */
    aw_err_t (*ioctl)(struct awbl_upp *_this, int cmd, va_list va );

    /**< 内部使用,驱动开发和应用开发都不需要关注 */
 ...
 *               AWBL_UPP_CMD_XFER_START:需要参数window_address,line_size_valid,line_size,line_count，等价使用awbl_upp_xfer_async()
...
aw_err_t awbl_upp_ioctl(struct awbl_upp *_this, int cmd, ... ); /* need to set msgq_id */

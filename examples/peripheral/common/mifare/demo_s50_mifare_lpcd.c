 ...
 *
 * - 操作步骤：
 *   1. 因为FM175XX芯片使用SPI1驱动，例程使用蜂鸣器鸣叫指示，所以要工程配置文件 aw_prj_params.h
 *      文件中打开以下设备:
 *      - AW_DEV_XXX_LPSPI3
    ...
    AW_FOREVER {
        ...
        if (AW_OK == ret) {
            ...
            if ((mifare.mifare.atqa & 0xC0) == 0x00) {
                /* 1重UID */
                mifare.mifare.cascal_level = 1;
            } else if ((mifare.mifare.atqa & 0xC0) == 0x40) {
                /* 2重UID */
                mifare.mifare.cascal_level = 2;
            } else if ((mifare.mifare.atqa & 0xC0) == 0x80) {
                /* 3重UID */
                mifare.mifare.cascal_level = 3;
            }

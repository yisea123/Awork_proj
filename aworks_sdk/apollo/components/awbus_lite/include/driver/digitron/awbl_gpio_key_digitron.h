...
struct awbl_gpio_key_digitron_param {
    ...
    const uint16_t                   *p_seg_pins;   /**< \brief 段码 GPIO驱动 引脚 */
    const uint16_t                   *p_com_pins;   /**< \brief 位码 GPIO驱动 引脚 */
...
aw_local aw_inline void awbl_gpio_key_digitron_data_send(struct awbl_gpio_key_digitron_dev *p_this, uint8_t idx)
{
	p_this->pfn_data_send(p_this, idx);
}

void awbl_gpio_key_digitron_dev_ctor(struct awbl_gpio_key_digitron_dev *p_this,
		                         struct awbl_gpio_key_digitron_param   *p_par,
		                         uint8_t                                disp_dev_id);

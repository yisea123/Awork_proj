 ...
 * - 141224 orz, first implementation.
 * \endinternal
 */
 ...
 * @{
 */
#define AW_MTD_FLAG_WRITEABLE     0x400  /**< device is writeable */
#define AW_MTD_FLAG_BIT_WRITEABLE 0x800  /**< single bits can be flipped */
#define AW_MTD_FLAG_NO_ERASE      0x1000 /**< no erase necessary */
#define AW_MTD_FLAG_POWERUP_LOCK  0x2000 /**< always locked after reset */
/** @} */
...
enum aw_mtd_oob_mode {
    ...
    /**
     * OOB data are automatically placed at the free areas which are defined
     * by the internal ECC layout.
     */
    AW_MTD_OOB_MODE_AUTO = 1,
    /**
     * OOB data are transferred as-is, with no error correction,
 ...
 * If \a fail_addr = AW_MTD_ERASE_FAIL_ADDR_UNKNOWN, the failure was not at the
...
struct aw_mtd_oob_ops {
    ...
    size_t ret_len;     /**< number of data bytes written/read */
    size_t oob_len;     /**< number of OOB bytes to write/read */
    size_t oob_ret_len; /**< number of OOB bytes written/read */

    /**
     * offset of OOB data in the OOB area (only relevant
     * when mode = AW_MTD_OPS_PLACE_OOB or AW_MTD_OPS_RAW)
     */
    off_t oob_offs;

    void *data_buf; /**< data buffer, if NULL only OOB data are read/written */
    void *oob_buf;  /**< OOB data buffer */
};
...
struct aw_mtd_info {
    ...
    uint_t              erase_size; /**< "Major" erase size for the device */
    /**
     * Minimal writable unit size. In case of NOR flash it is 1 (even
    ...
    /**
     * Read ops return -EUCLEAN if max number of bit flips corrected on any
     * one region comprising an ECC step equals or exceeds this value.
     * Settable by driver, else defaults to ecc_strength.
     */
    uint_t bitflip_threshold;

    /* max number of correctable bit errors per ECC step */
    uint_t ecc_strength;

    /* OOB layout description data */
...
bool_t aw_mtd_is_partition (struct aw_mtd_info *mtd);

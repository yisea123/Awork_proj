...
/******************************************************************************/
static int __usb_mem_sta (int argc, char *argv[], struct aw_shell_user *p_user);

aw_local uint32_t                   __g_usb_mem_alloc_cnt = 0;
aw_local uint32_t                   __g_usb_mem_map_cnt = 0;
aw_local struct aw_shell_cmd_list   __g_usb_mem_cl;
aw_local const struct aw_shell_cmd  __g_usb_mem_cmd_tbl[] = {
        {__usb_mem_sta,  "umem", "show USB memory status."}
};
...
void aw_usb_mem_lib_init (void *p_mem, size_t size)
{
    ...
    (void)aw_shell_register_cmds(
            &__g_usb_mem_cl,
            &__g_usb_mem_cmd_tbl[0],
            &__g_usb_mem_cmd_tbl[AW_NELEMENTS(__g_usb_mem_cmd_tbl)]);
#endif
}

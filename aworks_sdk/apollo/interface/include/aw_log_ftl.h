...
#define __LOG_FTL_DEV_NAME_MAX    32
    ...
    int                         sector_threshold;   /**< \brief 数据清除门槛，空闲扇区低于该值启动清除  */
    int                         newest_lsn;         /**< \brief 最新数据的扇区号  */
    int                         oldest_lsn;         /**< \brief 最旧的数据的块号  */

    bool_t                      init_flag;          /**< \brief 初始化标记  */
    bool_t                      wait_write;         /**< \brief 等待写入标记  */
    bool_t                      format_req;         /**< \brief 格式化磁盘请求  */
} aw_log_ftl_ctx_t;

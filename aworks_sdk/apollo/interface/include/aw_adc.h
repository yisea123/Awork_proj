 ...
 *
 *     aw_adc_client_init(&client, IMX28_HSADC_LRADCPIN0, FALSE);
 *
 ...
 * \brief   转换完成回调函数类型
 *
 * \param   p_cookie    用户初始化客户端传入的回调函数参数
 * \param   state       转换完成的状态：
 *                        AW_OK        : 转换成功
 *                        AW_ERROR     : 转换出错
 *
 * \note 处理转换数据前应该确保转换结束的状态为AW_OK。如果不为AW_OK，则后续转换序列
 *       将自动取消，不会再转换。通常情况下，转换出错可能是由于缓冲区的buf过小，使得
 *       buf切换需要很频繁，数据处理时间稍长，数据就溢出了。可以增大缓冲区的buf大小
 *       以增加buf切换的时间间隔。
 */

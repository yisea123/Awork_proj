...
#define am_lora_static_inline  static  __inline      /**< ARM编译器inline关键字 */

#elif defined (__ICCARM__)
#define am_lora_inline         inline
#define am_lora_static_inline   static inline         /**< IAR编译器inline关键字 */

#elif defined   (__GNUC__)
#define am_lora_inline         inline
#define am_lora_static_inline  static  inline        /**< GNU编译器inline关键字 */

#elif defined   (__TASKING__)
#define am_lora_inline         inline
#define am_lora_static_inline  static inline         /**< TASKING编译器inline关键字 */
...
/** \brief 求结构体成员的偏移*/
#define AM_LORA_OFFSET(structure, member)   ((size_t)(&(((structure *)0)->member)))
 ...
 * \brief 断言一个表达式
 *
 * 当表达式为真时这个宏不做任何事情，为假时会调用am_lora_assert_msg()输出断言信息，
 * 断言信息格式为：
 * 文件名：行号：表达式
 * \hideinitializer
 */
...
/* help macro */
#define _AM_LORA_VTOS(n)     #n                 /* Convert n to string */
#define _AM_LORA_SVAL(m)     _AM_LORA_VTOS(m)   /* Get string value of macro m */

#define am_lora_assert(e) \
    ((e) ? (void)0 : am_lora_assert_msg(__FILE__":"_AM_LORA_SVAL(__LINE__)":("#e")\n"))

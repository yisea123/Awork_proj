...
/**
 * \file
 * \brief USB device mass storage
 *
 * \internal
 * \par modification history:
 * - 1.00 17-04-27  xdn, first implementation
 * \endinternal
 */

#ifndef __AWBL_USBD_CDC_H
#define __AWBL_USBD_CDC_H
...
#define AW_USBD_CDC_R_STACK_SIZE         1024     /* 数据接收任务堆栈 */
...
/** \brief usb cdc virtual serial object */
typedef struct aw_usbd_cdc_vs {
    struct aw_usbd_fun          cfun;
    struct aw_usbd_pipe         intr;
    struct aw_usbd_fun          dfun;
    struct aw_usbd_pipe         in;
    struct aw_usbd_pipe         out;
    uint8_t                     id;            /* 设备id编号 */
    uint8_t                     name[32];
    aw_usbd_cdc_line_coding_t   com;
    aw_usb_task_handle_t        rtask;
    aw_usb_mutex_handle_t       mutex;
    aw_usb_sem_handle_t         r_sync_semb;
    aw_usb_sem_handle_t         rng_semb;
//    AW_TASK_DECL(               rtask,          /* 数据接收任务 */
//                                AW_USBD_CDC_R_STACK_SIZE);
//    AW_SEMB_DECL(               r_sync_semb);   /* 数据接收同步信号量 */
//    AW_SEMB_DECL(               rng_semb);      /* 缓存区数据可用信号量 */
//    AW_MUTEX_DECL(              mutex);         /* 设备互斥信号量 */
    struct aw_list_head         node;           /* 设备节点 */
    struct aw_rngbuf            r_rngbuf;       /* 数据接收环形缓冲区 */
    char                       *p_rbuf;
    void                       *p_swap_rbuf;    /* 接收缓冲区 */
...
aw_err_t aw_usbd_cdc_vs_write (struct aw_usbd_cdc_vs       *p_vs,
                               struct aw_usbd_trans        *p_trans);

aw_err_t aw_usbd_cdc_vs_read (struct aw_usbd_cdc_vs  *p_vs,
                              void                   *p_buf,
                              size_t                  nbytes,
                              size_t                 *p_act_len);

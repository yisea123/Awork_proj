...
/**
 * \file
 * \brief AWorks 文件系统操作例程
 *
 * - 本例程展示了AWorks 文件系统中大多数接口操作，以使用FAT文件系统操作SD卡为例。
 *
 * - 操作步骤：
 ...
 *
 * 3. 物理存储器：
 *    1) 以 SPI FLASH 为例，其mtd设备名字在aw_hwconf_spi_flash0.h文件中已指定，为"/sflash0",
 *       文件系统挂载时就需要这个名字
 ...
 * 并且文件名必须符合8.3原则，即8个文件名字符，3个扩张名字符
 *
 * 目前已有文件系统 "vfat", "lffs", "txfs", 本历程使用vfat操作SD卡
 */
 ...
 * \return 无
 */
void  demo_fs_entry (bool_t format)
{
    aw_kprintf("\n\n\n**********************************************\n");

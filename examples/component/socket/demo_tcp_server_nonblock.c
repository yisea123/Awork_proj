...
/**
 * \addtogroup demo_if_tcp_server_nonblock
...
void demo_tcp_server_nonblock_entry (void)
{
    ...
    for (;;) {
        ...
        if (client_sock > 0) {
            ...
            /* 设置保活时间 */
            if (keep_idle > 0) {
                setsockopt(client_sock, SOL_SOCKET, SO_KEEPALIVE, (void *) &sockopt_on,
                        sizeof(int));
                setsockopt(client_sock, IPPROTO_TCP, TCP_KEEPIDLE, (void *) &keep_idle,
                        sizeof(keep_idle));
                setsockopt(client_sock, IPPROTO_TCP, TCP_KEEPINTVL, (void *) &keep_interval,
                        sizeof(keep_interval));
                setsockopt(client_sock, IPPROTO_TCP, TCP_KEEPCNT, (void *) &keep_count,
                        sizeof(keep_count));
            }

 ...
 * \brief 通用PBUF组件接口.
 *
 * 本模块提供用于通信中对数据缓冲区资源的管理，数据缓冲区具备引用计数机制，适用于多个模块
 * 或者通道共用相同缓存资源时，对缓冲区资源的管理,提高缓冲区资源的使用效率，降低内存的开销。
 *
 ...
 *
 *        AW_PBUF_TYPE_POOL 类型的PBUF数据缓冲区和PBUF头由内存池提供，该内存池的内存
 *        和池分块大小由对象初始化时指定，由于池分块大小是固定的，所以当用户要求分配一个较大PBUF
 *        时组件会申请多个内存池分块并把它们链起来返回首个PBUF的指针，其分配与释放由组件负责，
 *        该类型的主要应用于驱动程序。
 *
 *        AW_PBUF_TYPE_ROM/REF 类型PBUF的PBUF头由内存池提供，该内存池的内存在对象初
 *        始化的时候指定，而PBUF的数据缓冲区由用户提供，在使用aw_pbuf_alloc接口时可以指
 *        定PBUF数据缓冲区，所以这两种类型的PBUF的PBUF头的申请与释放由组件负责，数据缓冲
 *        区的申请释放由用户负责，当调用aw_pbuf_free后这两种类型的PBUF的引用计数为0时，
 *        组件会释放PBUF头，这两种类型唯一不同是数据缓冲区存储区域,AW_PBUF_TYPE_ROM类型
 *        缓冲区在ROM中。
 */
typedef enum {

    /** \brief 数据缓冲区和PBUF头都在内存堆中分配（组件提供），适用于应用程序与协议栈 */
    AW_PBUF_TYPE_RAM = 0,

    /** \brief 数据缓冲区由用户提供（ROM），PBUF头由组件提供（内存池），适用于应用程序 */
    AW_PBUF_TYPE_ROM,

    /** \brief 数据缓冲区由用户提供，PBUF头由组件提供（内存池），适用于应用程序 */
    AW_PBUF_TYPE_REF,
...
struct aw_pbuf {
    ...
    /** \brief 当前PBUF及之后链上所有PBUF（同属一个包）数据缓冲区长度和，在通信过程中，
     *         一个数据包可能由多个PBUF链接而成，这个变量记录当前PBUF所属的数据包还有多
     *         少数据存储空间。规则：ret_len(p) = ret_len(p->next) + len(p->next)
     */
    size_t               tot_len;
...
struct aw_pbuf_pool {
    ...
    /** \brief 内存池，用于分配AW_PBUF_TYPE_POOL类型的PBUF节点和数据缓冲区需要的内存 */
 ...
 *
 * \note 调整后的空间可存储其他信息，调整范围在数据缓冲区空间之内,这个接口可用于裁剪数据
 *       帧的头部
 */
 ...
 *
 * \note 调用此接口后p_tail引用无效，这个接口将p_tail加入p_head包中。注意p_head和
 *       p_tail之间不能是交叉的，也就是说如果p_head和p_tail链了相同的PBUF,那么p_head和
 *       p_tail不能使用这个接口。
 */
 ...
 * \retval -AW_ENOTSUP  p_tail已经存在链p_head中，操作不允许
 *
 * \note p_pkt_fifo是一个包队列的头，p_pkt包加入包队列p_pkt_fifo中，调用此接口后p_pkt引用无效。
 *       这个接口与上面aw_pbuf_pkt_expand不同，一个数据包可能由一个PBUF组成的，也可能
 *       是有多个PBUF组成的，aw_pbuf_pkt_expand接口是将p_pkt链加入包p_pkt_fifo中，而
 *       aw_pbuf_pkt_chain是将包加入包队列中。注意p_pkt_fifo和p_pkt之间不能是交叉的，也就
 *       是说如果p_pkt_fifo和p_pkt链了相同的PBUF,那么p_pkt_fifo和p_pkt不能使用这个接口。
 */

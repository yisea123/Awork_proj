...
/**
 * \addtogroup demo_if_unaligned_access
...
aw_local void test_unaligned_access_helper ()
{
    volatile test_pack             *p;

    p = (volatile test_pack *)aw_cache_dma_align(sizeof(test_pack),4);
    if(NULL == p){
    ...
    aw_cache_dma_free((void *)p);

    return ;
}

aw_local void test_memcpy_helper ()
{
    volatile char             *p;

    p = (volatile char *)aw_cache_dma_align(1024,4);
...
void demo_unaligned_access_entry ()
{
    aw_kprintf("\n");

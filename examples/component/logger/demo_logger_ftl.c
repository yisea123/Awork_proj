...
void demo_logger_ftl_entry (void)
{
    ...
    /* 初始化 ftl 日志存储
     * "/sflash0" 在 awbl_hwconf_spi_flash0.h中定义
     */
    __g_p_logger = aw_log_ftl_ctor(&__g_ftl_ctx, "/sflash0", TRUE);
    ...
    err = aw_log_export(__g_p_logger,
                        0,
                        __g_buffer,
                        (__g_buffer_size > log_size)?log_size:__g_buffer_size);
    if (err < 0) {
...
__log_ftl_task_entry_end:
    /* 卸载日志器 */
    aw_logger_unregister(__g_p_logger);
    return ;
}
/** [src_log_ftl] */

 ...
 *       TIME  对应AW_LOG_FMT_TIME，表示时间信息，外部用户实现函数#aw_loglib_time_get()可以修改显示消息的格式
 *       TASK  对应AW_LOG_FMT_TASK_INFO，表示任务信息，外部用户实现函数#aw_loglib_task_info_get()可以修改显示任务信息的格式
 ...
 *       等级（level）：消息等级不大于设置的过滤值（优先级相同或更高）的消息才会输出，例如过滤level设置为AW_LOG_LVL_ERROR，
 *       则只有消息级别为AW_LOG_LVL_ERROR和AW_LOG_LVL_ASSERT的消息才能输出。

...
#define AW_CAN_ERR_NONE                       0                      /**< \brief 无错误 */
#define AW_CAN_ERR_BASE                       0                      /**< \brief 错误基号 */
#define AW_CAN_ERR_BUSY                      (AW_CAN_ERR_BASE + 80)  /**< \brief 总线忙 */
#define AW_CAN_ERR_NOT_INITIALIZED           (AW_CAN_ERR_BASE + 81)  /**< \brief 设备未初始化 */
#define AW_CAN_ERR_NONE_DRIVER_FUNC          (AW_CAN_ERR_BASE + 82)  /**< \brief 无注册驱动函数 */
#define AW_CAN_ERR_ILLEGAL_CHANNEL_NO        (AW_CAN_ERR_BASE + 83)  /**< \brief 非法通道号 */
#define AW_CAN_ERR_ILLEGAL_CHANNEL_MODE      (AW_CAN_ERR_BASE + 84)  /**< \brief 非法通道模式 */
#define AW_CAN_ERR_ILLEGAL_CTRL_TYPE         (AW_CAN_ERR_BASE + 85)  /**< \brief 非法的控制器类型 */
#define AW_CAN_ERR_INCOMPATIBLE_CTRL_TYPE    (AW_CAN_ERR_BASE + 86)  /**< \brief 控制器类型不匹配 */
#define AW_CAN_ERR_CTRL_NOT_START            (AW_CAN_ERR_BASE + 87)  /**< \brief 控制器未启动 */
#define AW_CAN_ERR_NO_OP                     (AW_CAN_ERR_BASE + 88)  /**< \brief 无可操作资源 */
#define AW_CAN_ERR_ILLEGAL_DATA_LENGTH       (AW_CAN_ERR_BASE + 89)  /**< \brief 非法数据长度 */
#define AW_CAN_ERR_ILLEGAL_MASK_VALUE        (AW_CAN_ERR_BASE + 90)  /**< \brief 非法的掩码值 */
#define AW_CAN_ERR_INVALID_PARAMETER         (AW_CAN_ERR_BASE + 91)  /**< \brief 无效参数 */
#define AW_CAN_ERR_ILLEGAL_OFFSET            (AW_CAN_ERR_BASE + 92)  /**< \brief 非法偏移值 */
#define AW_CAN_ERR_SLEEP_IN_CFG_MODE         (AW_CAN_ERR_BASE + 93)  /**< \brief 配置模式中休眠 */
#define AW_CAN_ERR_CANNOT_SET_WAKEUPINT      (AW_CAN_ERR_BASE + 94)  /**< \brief 无法设置唤醒中断 */
#define AW_CAN_ERR_CANNOT_SET_BOFFINT        (AW_CAN_ERR_BASE + 95)  /**< \brief 无法设置总线关闭中断 */
#define AW_CAN_ERR_CANNOT_SET_ERRINT         (AW_CAN_ERR_BASE + 96)  /**< \brief 无法设置错误中断 */
#define AW_CAN_ERR_BAUDRATE_ERROR            (AW_CAN_ERR_BASE + 97)  /**< \brief 波特率错误 */
#define AW_CAN_ERR_ABORT_TRANSMITTED         (AW_CAN_ERR_BASE + 98)  /**< \brief 终止传输 */
#define AW_CAN_ERR_TRANSMITTED_TIMEOUT       (AW_CAN_ERR_BASE + 99)  /**< \brief 传输超时 */
#define AW_CAN_ERR_BUS_FAULT                 (AW_CAN_ERR_BASE + 100) /**< \brief 总线错误 */
#define AW_CAN_ERR_RXD_BUFF_UNUSED           (AW_CAN_ERR_BASE + 101) /**< \brief 无接收缓存区 */
#define AW_CAN_ERR_NONE_RCV_FUNC             (AW_CAN_ERR_BASE + 102) /**< \brief 无注册接收函数 */
#define AW_CAN_ERR_NOT_SUPPORT               (AW_CAN_ERR_BASE + 103) /**< \brief 不支持该操作 */
#define AW_CAN_ERR_UNKNOWN_ERROR             (AW_CAN_ERR_BASE + 255) /**< \brief 未知错误 */
...
#define AW_CAN_MSG_FLAG_SEND_TYPE_MASK       0x000F    /**< \brief 消息标志中发送类型掩码 */
#define AW_CAN_MSG_FLAG_CTRL_TYPE_MASK       0x00F0    /**< \brief 消息标志中报文类型掩码 */

#define AW_CAN_MSG_FLAG_SEND_TYPE_SHIFT      0x0000    /**< \brief 消息标志中发送类型的偏移位 */
#define AW_CAN_MSG_FLAG_CTRL_TYPE_SHIFT      0x0004    /**< \brief 消息标志中报文类型的偏移位 */
...
typedef struct aw_can_transfer {
    ...
    int               status;         /**< \brief 当前传输块的状态如下: */
                                      /**< \brief -AW_ENOTCONN:未排队*/
                                      /**< \brief -AW_EISCONN:排队*/
                                      /**< \brief -AW_EINPROGRESS:处理*/
                                      /**< \brief -AW_ETIME:超时*/
                                      /**< \brief -AW_ENETDOWN:总线关闭，设备停止*/
                                      /**< \brief AW_OK:完成*/
...
/** \brief 接收数据的回调函数 */
typedef bool_t (*aw_can_proto_rx_callback_t) (int chn, aw_can_msg_t* p_msg);

/** \brief 发送数据的回调函数 */
typedef bool_t (*aw_can_proto_tx_callback_t) (int chn);

/** \brief 总线错误状态的回调函数 */
typedef bool_t (*aw_can_bus_err_sta_callback_t) (
        int chn, aw_can_bus_err_t err, aw_can_bus_status_t status);

/** \brief 设备唤醒的回调函数 */
typedef bool_t (*aw_can_dev_wakeup_callback_t) (int chn);
...
typedef struct aw_can_app_callbacks {
     ...
     * 函数返回值决定接收到的报文是否存入rngbuf， TRUE:不保存，FALSE:保存
     */
    aw_can_proto_rx_callback_t       proto_rx_cb;

    /**
     * \brief  总线发送完成回调函数地址
     * 函数返回值决定是否自动处理发送报文， TRUE:回调自行处理，FALSE:自动处理发送
     */
    aw_can_proto_tx_callback_t       proto_tx_cb;

    /**
     * \brief  应用层错误及总线状态处理回调函数地址
     * 函数返回值决定错误消息是否存入rngbuf， TRUE:不保存，FALSE:保存
     */
    aw_can_bus_err_sta_callback_t    bus_err_sta_cb;

    /**
     * \brief  总线唤醒回调函数地址
     * 函数返回值决定唤醒消息是否存入rngbuf， TRUE:不保存，FALSE:保存
     */
...
aw_can_err_t aw_can_filter_table_get (
        int chn, uint8_t *p_filter_buff, size_t *p_length);

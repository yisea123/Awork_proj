...
#define __PUT_UINT16(buf, var, n)  ((buf[n]   = (uint8_t) (( var) & 0x00ff)),       \
                                    (buf[n+1] = (uint8_t) (((var) & 0xff00) >> 8)))

#define __PUT_UINT32(buf, var, n)  ((buf[n]   = (uint8_t) ( (var) & 0x000000ff)),       \
...
#define __VALUE_TO_BITS(value, start_bit, len)   (((value) << (start_bit)) & __BITS_MASK(start_bit, len))
#define __VALUE_FROM_BITS(value, start_bit, len) (((value) & __BITS_MASK((start_bit), (len))) >> (start_bit))
...
extern struct am_lrnet_global {
    ...
    uint32_t p2p_ms_max;        /**< time of sending the biggest packet between router */
...
{
    return (AM_LRNET_ADDR_BROADCAST == addr) || AM_LRNET_ADDR_IS_GROUP_CAST(addr);
}

...
typedef struct aw_lffs_page_spare {
	struct aw_lffs_tags  tag;			//!< page tag
	int                  expired:1;
...
struct aw_lffs_block_info {
	...
	struct aw_lffs_page_spare *spares;	//!< page spare info array
...
aw_err_t aw_lffs_block_info_cache_init (aw_lffs_device_t *dev, int maxCachedBlocks);
...
aw_err_t aw_lffs_block_info_load (aw_lffs_device_t *dev, aw_lffs_block_info_t *work, int page);

/** find block info cache */
aw_lffs_block_info_t * aw_lffs_block_info_find (aw_lffs_device_t *dev, int block);

/** get block info cache, load it on demand */
aw_lffs_block_info_t * aw_lffs_block_info_get (aw_lffs_device_t *dev, int block);

/** put info cache back to pool, should be called with #aw_lffs_block_info_tGet in pairs. */
void aw_lffs_block_info_put (aw_lffs_device_t *dev, aw_lffs_block_info_t *p);

/** explicitly expire a block info cache */
void aw_lffs_block_info_expire (aw_lffs_device_t *dev, aw_lffs_block_info_t *p, int page);

/** no one hold any block info cache ? safe to release block info caches */
bool_t aw_lffs_block_info_is_all_free (aw_lffs_device_t *dev);

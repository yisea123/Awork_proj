...
#define NET_TOOLS_RAISE_ERROR(e)    e = TRUE; break

#define NET_TOOLS_SHELL_REQ_HELP(argc, argv) ((1 == argc) \
        && (('?' == argv[0][0]) \
                || !strcmp("help", argv[0]) \
                || !strcmp("--help", argv[0])))

#define NET_TOOLS_CHECK_SHELL_ARGC(exp) do { \
        if (!(exp) || \
                NET_TOOLS_SHELL_REQ_HELP(argc, argv)) { \

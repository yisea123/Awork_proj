 ...
 *
 * 本驱动为 TPS08U 模块的操作接口库，驱动的底层使用 SPI 标准接口来操作 TPS08U 模块
 *
 * TPS08U 共有 8 个通道(CH1~CH8)，每 2 个通道为一组，如 CH1 与 CH2 为一组，CH3 与
 * CH4 为一组，以此类推。每组通道可以自由配置为电压通道或者电流通道。
 *
 * 在使用传感器接口(aw_sensor.h)操作 TPS08U 的时候，TPS08U 提供了 8 个电压/电流通道，
 * 通道 ID 为设备信息中的 start_id ~ start_id + 7。
 *
 * 用户可以手动通过 aw_sensor_data_get() 获取通道数据，也可以使用 aw_sensor_attr_set()
 * 配置采样频率，推荐的采样频率为 50Hz，由于 TPS08U 所有通道数据是一起更新的，所以配置采样
 * 频率时，通道参数被忽略。在配置采样频率后，系统将自动按照采样频率获取传感器所有使能的通道
 * 的采样值，并存于内部缓存中。若用户设置了数据准备就绪触发，则每次数据采样结束后，都将触
 * 发用户回调函数，用户可在回调函数中使用 aw_sensor_data_get() 获取通道数据。
 *
 * 由于 TPS08U 本身不支持触发功能，驱动中的数据准备就绪触发、门限触发以及增量触发都是通过
 * 软件模拟实现的，所以需要配置采样频率大于 0 后，配置好的触发回调函数才会被正常回调。
 *
 * 还需要注意的是，当采样频率配置大于 0 的时候，系统会自动获取传感器值，此时使用其它接口操作
 * TPS08U 会返回错误 -AW_EBUSY，所以建议用户配置完成需要配置的选项之后再配置采样频率。
 *
 * \internal
...
struct awbl_tps08u_dev {
    ...
    aw_sensor_trigger_cb_t  pfn_cb[8];          /**< \brief 触发时执行的回调函数的指针 */
    void                   *p_arg[8];           /**< \brief 回调函数用户参数 */
    uint8_t                 trigger;            /**< \brief 触发使能位，每位对应一个通道 */

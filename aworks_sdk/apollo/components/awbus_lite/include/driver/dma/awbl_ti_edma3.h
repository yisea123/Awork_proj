...
typedef struct awbl_ti_edma3_gblcfg_params {
    ...
    /** \brief
     * 在多主机系统中(e.g. ARM + DSP)，本选项用于区分主机和从机。只有主机被允许
     * 编程全局寄存器 (例如，队列优先级、队列水印门限、错误寄存器等等)
     */
//    bool_t                is_master;
     ...
     */
    bool_t                dma_ch_param_map_exists;

    /** \brief 内存保存特性是否存在 */
    bool_t                mem_protection_exists;

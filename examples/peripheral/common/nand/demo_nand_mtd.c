...
static size_t              g_burn_offset = 490*2048*64;
static uint8_t            *gp_rw_buf, *gp_tmp_buf;

/*分区大小*/
#define BLOCK_SIZE           2048
/*分区数量*/
#define BLOCK_COUNT          1280
...
{
    /*
     * note: mtd的名字("/dev/rootfs")在 nandflash 的硬件配置(awbl_hwconf_imxxxxxxx_nandflash.h)里查找
     */
    gp_mtd = aw_mtd_dev_find("/dev/rootfs");
    if (gp_mtd == NULL) {

...
typedef struct awbl_ts_drv_funcs {
    ...
    /* 设置是否锁屏 */
    void (*pfn_lock_set) (struct awbl_ts_service *p_serv, bool_t lock);
...
typedef struct awbl_ts_service {
    ...
    union {
        aw_ts_lib_factor_t factor;            /**< \brief five point calibrate factor */
        aw_ts_lib_minmax_t xy_minmax[2];      /**< \brief two point calibrate factor  */
    };
    ...
    uint32_t  x_last_sample[AW_MULTI_TS_CNT]; /**< \brief x last sample data buffer */
    uint32_t  y_last_sample[AW_MULTI_TS_CNT]; /**< \brief y last sample data buffer */
 ...
 * \return AW_OK           : 初始化成功
 *         -AW_EINVAL      : 参数错误
 *         -AW_ENOTSUP     : 当前触摸配置的多重触摸点数超过系统支持的最大触摸点数 AW_MULTI_TS_CNT
 *
 */

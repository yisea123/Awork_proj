 ...
 * 使用本服务需要包含头文件
 * \code
 * #include "aw_usbd_ms.h"
 * \endcode
 *
 * \internal
 * \par Modification History
 * - 1.00 16-08-18  deo, first implementation
 * \endinternal
 */
...
struct aw_usbd_ms_lun {
    ...
    bool_t                  inf_valid; /**< \brief sense info是否有效 */
    bool_t                  ro;        /**< \brief 是否只读 */
    bool_t                  rm;        /**< \brief 是否可移除 */
    bool_t                  pv;        /**< \brief prevent标志 */
...
struct aw_usbd_ms {
    ...
    bool_t                  buf_auto;   /**< \brief 是否动态分配内存 */

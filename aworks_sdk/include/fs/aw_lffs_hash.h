 ...
 *          ELSE: 查找到hash结点指针
 */
struct aw_lffs_hash_node *aw_lffs_hash_next_find (struct aw_lffs_hash_table  *p_hash,
                                                  struct aw_lffs_hash_node   *p_node);

/**
 * \brief 获取和p_node的关键字相同的下一个结点，p_node为空则获取该关键字的第一个节点
 *
 * \param[in]       p_hash      hash表地址
 * \param[in]       p_node      当前结点，可为NULL
 * \param[in]       p_key       关键字指针
 ...
 *          ELSE: 查找到hash结点指针
 */
struct aw_lffs_hash_node *aw_lffs_same_hash_next_find (struct aw_lffs_hash_table  *p_hash,
                                                       struct aw_lffs_hash_node   *p_node,
                                                       const void                 *p_key);
 ...
 * \retval  该链表中的元素数量
 */
int aw_lffs_hash_list_num_get (struct aw_lffs_hash_table  *p_hash, int list_index);

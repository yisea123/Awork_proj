 ...
 *
 * - 操作步骤：
 *   1. Modbus主站使用串行模式时（RTU/ASCII模式），本例程是使用串口3（COM2）
 *      作为Modbus主站通讯串口，故需在aw_prj_params.h头文件里使能
 *      - AW_COM_CONSOLE
 *      - 调试串口
 *      - 串口3
 *   2. 将串口3(TXD、RXD)通过USB转串口设备和PC相连；
 *      打开Modubs从站上位机软件（如Modbus Slave）,设置串口参数（115200的波特率、偶校验、1个停止位），
...
/**
 * \addtogroup demo_if_modbus_master
...
void demo_modbus_master_entry (void)
{
    ...
    aw_mb_master_serial_params_t ser_params = {COM2,
                                               AW_MB_PAR_EVEN,
                                               115200,
                                               FALSE};

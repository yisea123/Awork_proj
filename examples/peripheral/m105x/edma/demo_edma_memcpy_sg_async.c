...
/**
 * \addtogroup demo_if_edma_memcpy_sg_async
...
void demo_edma_memcpy_sg_async_entry(void)
{
    ...
    chan = imx1050_edma_alloc_chan(&data);
    if(chan == NULL){
    ...
    ret = imx1050_edma_control(chan, DMA_SLAVE_CONFIG, (unsigned long)&cfg);
    if(ret != AW_OK){
    ...
    /* 准备一个传输  */
    ret = imx1050_edma_prep_memcpy_sg(chan, dst_list, 4, src_list, 4, 0);
    if(ret != AW_OK){
        AW_INFOF(("imx1050_edma_prep_memcpy_sg error!\r\n"));
        return ;
    }
    ...
    /* 开始传输 */
    ret = imx1050_edma_chan_start_async(chan, \

...
#define  AW_SDIO_DEV_STAT_UNINSTALL    (1 << 1)     /**< \brief dev uninstalled */
    bool_t                          changed;        /**< \brief is changed */
    bool_t                          removed;        /**< \brief is removed */
#ifdef SDIO_TRANS_ERRCNT_ENABLE
...
#define AWBL_SDIO_CON_DEV_DYNAMIC     1       /**< \brief 连接动态设备 */
    ...
    bool_t                  crc_en;           /**< \brief CRC校验使能 */
    bool_t                  dbl_rate_en;      /**< \brief 倍速模式使能 */
...
struct sdio_func_tuple {
    ...
    uint8_t                 code;
    uint8_t                 size;
    uint8_t                 data[]; //C99 flexible array(incomplete type)
//    uint8_t                 data[0]; //GNU C zero length array
};
...
typedef struct awbl_sdio_cis {
    uint16_t    tplmid_manf;         /**< \brief SDIO Card manufacturer code  (0x20) */
    uint16_t    tplmid_card;         /**< \brief manufacturer information     (0x20) */
    uint16_t    tplfe_max_blk_size;  /**< \brief function max block size      (0x22) */
    uint32_t    tplfe_max_speed;     /**< \brief max block size, only in func0(0x22) */
}awbl_sdio_cis_t;
...
typedef struct awbl_sdio_fbr {
    uint8_t interface;            /**< \brief Standard Function interface code */
    uint8_t  ext_interface_code;  /**< \brief extension of the SDIO Standard Function */
    bool_t   csa_support;         /**< \brief csa suport */
    uint32_t cis_addr;            /**< \brief func cis data addr */
    uint32_t csa_addr;            /**< \brief pointer to the desired byte in the CSA */
    uint8_t  csa_access_window;   /**< \brief func csa data addr */
    uint16_t blk_size;            /**< \brief function block size */
}awbl_sdio_fbr_t;
...
#define AWBL_SDIO_CLASS_BT_AMP      0x09    /**< \brief Type-A bluetooth AMP interface */
#define AWBL_SDIO_CLASS_EXT         0x0F    /**< \brief Not assigned, reserved for future use */
 ...
 * \param[in] t     timeout obj
 */
aw_local aw_inline bool_t awbl_sdio_timeout (struct awbl_sdio_timeout_obj *p_t)
{
 ...
 * @return          [description]
 */
aw_err_t awbl_sdio_host_get_port_status (struct awbl_sdio_host *p_host, uint8_t *p_status);
 ...
 * @return          [description]
 */
aw_err_t aw_sdio_host_send_status (struct awbl_sdio_host *p_host, uint32_t rca, uint32_t *p_status);
 ...
 * @return        [description]
 */
aw_err_t aw_sdio_host_send_op_cond (struct awbl_sdio_host *p_host, uint32_t ocr, uint32_t *p_rocr);
 ...
 * \retval -AW_ENOTSUP      : HOST不支持该功能
 *
 * \note 此接口传输数据不会使用数据线，而是使用CMD线，适用于数据线未初始化时对SDIO卡寄存
 *       器操作
 */
 ...
 * \param[in]  op_flg       : 地址自增标志，程序每读一个字节的数据地址是否自动增加，
 *                            若为TRUE每读取一个字节地址都会自动加1，若为FALSE地址将
 *                            一直不会变。
 *
 * \retval AW_OK            : 传输成功
 * \retval -AW_ENOMEM       : 内存空间不足
 ...
 *
 * \param[in] p_host        : SDIO总线HOST
 * \param[in] func_num      : 要设置块大小的功能号，0~7, 0为CIA区域，1~n表示不同的功
 *                            能，一张SDIO卡最多有1~6个IO功能
 * \param[in] blk_len       : 块大小，每个功能可设置的最大块大小可以从CIS区域读出，可
 *                            调用读CIS的接口获取可设置的最大块大小，blk_len应小于等
 *                            于最大块大小
 *
 * \retval AW_OK            : 传输成功
 * \retval -AW_ENOMEM       : 内存空间不足
 ...
 *
 * \param[in] p_host        : SDIO总线HOST
 * \param[in] func_num      : 要取消传输的功能号，0~7, 0为CIA区域，1~n表示不同的功能，
 *                            一张SDIO卡最多有1~6个IO功能
 *
 * \retval AW_OK            : 传输成功
 * \retval -AW_ENOMEM       : 内存空间不足
 ...
 *
 * \param[in]  p_host       : SDIO总线HOST
 * \param[in]  func_num     : 功能号，0~7, 0为CIA区域，1~n表示不同的功能，一张SDIO卡
 *                            最多有1~6个IO功能
 * \param[in]  addr         : SDIO卡func_num功能地址，从该地址起读取数据,地址范围为
 *                            0x0~0x1ffff
 * \param[out] p_buf        : 数据缓冲区
 ...
 * \param[in]  op_fix       : 地址自增标志，程序每读取一个字节的数据地址是否自动增加，
 *                            若为TRUE每写入一个字节地址都会自动加1，若为FALSE地址将
 *                            一直不会变
 *
 * \retval AW_OK            : 传输成功
 * \retval -AW_ENOMEM       : 内存空间不足
 ...
 * \param[in]  func_num     : 功能号，0~7, 0为CIA区域，1~n表示不同的功能，一张SDIO卡
 *                            最多有1~6个IO功能
 * \param[in]  addr         : SDIO卡func_num功能地址，从该地址起写入数据,地址范围为
 *                            0x0~0x1ffff
 * \param[out] p_buf        : 数据缓冲区
 * \param[in]  sector_size  : 数据块大小
 * \param[in]  sector_cnt   : 写入数据块个数
 * \param[in]  op_fix       : 地址自增标志，程序每写一个字节的数据地址是否自动增加，若
 *                            为TRUE每写入一个字节地址都会自动加1，若为FALSE地址将一
 *                            直不会变。
 *
 * \retval AW_OK            : 传输成功
 * \retval -AW_ENOMEM       : 内存空间不足
 ...
 * \param[in]  op_fix       : 地址自增标志，程序每读一个字节的数据地址是否自动增加，若
 *                            为TRUE每读取一个字节地址都会自动加1，若为FALSE地址将一
 *                            直不会变。
 *
 * \retval AW_OK            : 传输成功
 * \retval -AW_ENOMEM       : 内存空间不足
 ...
 * \param[in]  func_num     : 功能号，0~7, 0为CIA区域，1~n表示不同的功能，一张SDIO卡
 *                            最多有1~6个IO功能。
 * \param[in]  addr         : SDIO卡func_num功能地址，从该地址起写入数据地址范围为
 *                            0x0~0x1ffff
 * \param[out] p_buf        : 数据缓冲区

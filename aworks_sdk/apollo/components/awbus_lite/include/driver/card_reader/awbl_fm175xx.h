 ...
 * \return AW_OK
 */
aw_err_t awbl_fm175xx_crc_set (uint8_t id, bool_t is_tx, bool_t is_en);
 ...
 * @{
 */
#define AWBL_FM175XX_CMD_IDLE        (0x00)  /* idle */
#define AWBL_FM175XX_CMD_CALCCRC     (0x03)  /* activate crc coprocessor */
#define AWBL_FM175XX_CMD_TRANSMIT    (0x04)  /* transmit data in fifo */
#define AWBL_FM175XX_CMD_NOCMDCHANGE (0x07)  /* change Command register but un-effect the primitive */
#define AWBL_FM175XX_CMD_RECEIVE     (0x08)  /* activate receiver */
#define AWBL_FM175XX_CMD_TRANSCEIVE  (0x0C)  /* transmit fifo data and activate receiver */
#define AWBL_FM175XX_CMD_AUTHENT     (0x0E)  /* execute M1 security authentication */
#define AWBL_FM175XX_CMD_SOFTRESET   (0x0F)  /* software reset */
/* @} */
...
typedef struct fm175xx_cmd {
    ...
    bool_t         snd_finish;    /**< \brief 发送完成标志 */
} awbl_fm175xx_cmd_t;
...
#define AWBL_FM175XX_LPCD_TX2RFEN           (1)   /**< \brief TX2输出使能，且与TX1反向 */
#define AWBL_FM175XX_LPCD_CWN               (0x1) /**< \brief N管驱动为大驱动 */
#define AWBL_FM175XX_LPCD_CWP               (0x3) /**< \brief P管驱动能力 */
#define AWBL_FM175XX_LPCD_MODE              (0x0) /**< \brief 低功耗模式控制 */
#define AWBL_FM175XX_TIMER1_CFG             (3)   /**< \brief T1阶段配置 */
#define AWBL_FM175XX_TIMER2_CFG             (13)  /**< \brief T2阶段配置 */
#define AWBL_FM175XX_TIMER3_CFG             (11)  /**< \brief T3阶段配置 */
#define AWBL_FM175XX_VMID_BG_CFG            (8)   /**< \brief Vmid配置 */
#define AWBL_FM175XX_AUTO_WUP_EN            (1)   /**< \brief 使能自动唤醒功能 */
#define AWBL_FM175XX_AUTO_WUP_CFG           (1)   /**< \brief 自动唤醒时间为12S，是否生效取决于自动唤醒是否使能 */
 ...
 * \return
 */
aw_err_t awbl_fm175xx_lpcd_ctrl (uint8_t id, bool_t is_enter);
 ...
 * @{
 */
#define AWBL_FM175XX_PICC_REQ_IDLE  (0) /**< \brief 仅请求所有空闲卡（不包括处于HALT状态的卡） */
#define AWBL_FM175XX_PICC_REQ_ALL   (1) /**< \brief 请求所有卡（包括处于HALT状态的卡） */
 ...
 * @{
 */
#define AWBL_PICC_CMD_HALT   (0x50)  /**< \brief halt */

/** \brief TYPE A 操作命令 */
#define AWBL_PICC_CMD_REQA   (0x26)  /**< \brief request */
#define AWBL_PICC_CMD_WUPA   (0x52)  /**< \brief wakeup */
 ...
 *       记录冲突情况，默认情况下该位段在无冲突发生时应该只有一个位置位的，但是不影响后续的操作，
 *       因为卡片已被唤醒。
 *
 * \return errno
 */
 ...
 * \return errno
 */
aw_err_t awbl_fm175xx_picca_select (uint8_t  id,    uint8_t  cl,
                                    uint8_t *p_dat, uint8_t *p_sak);
...
#define AWBL_PICCB_AFI_MAIN_ALL                 (0U) /**< \brief All main Family */
...
#define AWBL_PICCB_AFI_SUB_ALL                  (0U) /**< \brief All sub Family */
...
typedef struct awbl_piccb_attrib {
    bool_t   is_eof;    /**< \brief 是否需要帧结束标识，FALSE : 不需要；TRUE : 需要 */
    bool_t   is_sof;    /**< \brief 是否需要帧起始标识，FALSE : 不需要；TRUE : 需要 */
...
/**
 * \brief 读取UID数据
 * \note 需要获取uid数据时，用户需要确保p_uid指向的缓存能够容纳uid数据的长度，PICCB类型卡的UID
 *       长度为10字节。
 *
 * \param[in] id     FM175XX 设备 ID
 ...
 *
 * \note 用户调用该接口时，确保数据缓冲区的大小和blk_size指定的大小相符合应该足够容纳一块数据大小
 *       如S50的一块数据为16字节，则p_buf至少16字节大小
 * \return errno
 */
 ...
 *
 * \note 只针对value block有效，执行该命令前需要将块（block）初始化成value block
 *       执行该函数之后，数据仅仅保存在transfer buffer中，需要执行 awbl_mifare_transfer
 *       之后才会把数据重新保存到block中
 *
 * \note errno
 */
 ...
 *
 * \note 可以通过该接口对一个扇区的秘钥值进行修改，需要格外注意，修改某个秘钥值或者访问位时，
 *       需要确保不修改的位域其值保持不变，如：
 *       KEYA秘钥：0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF
 *       KEYB秘钥：0x11, 0x11, 0x11, 0x11, 0x11, 0x11
 *       访问位：0xFF, 0x07, 0x80
 *       只修改KEYA秘钥时，传入的参数中，p_keyb存储的秘钥值为上述KEYB秘钥值，p_ab存储的
 *       访问位为上述访问位值，确保这两部分在修改前后保持不变。
 * \return errno
 */

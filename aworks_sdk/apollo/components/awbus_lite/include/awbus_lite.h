...
struct awbl_bustype_info {
    ...
    /** \brief 检查设备和驱动是否匹配 */
    bool_t (*pfunc_bustype_devmatch)(const struct awbl_drvinfo *p_drv,
...
struct awbl_buspresent {
    struct awbl_buspresent         *p_next;    /* next bus instance */
    const struct awbl_bustype_info *p_bustype; /* bus type */
    struct awbl_dev     *p_ctlr;               /* pointer to bus controller*/
    struct awbl_dev     *p_inst_list;          /* pointer to instance list */
    struct awbl_dev     *p_orphan_list;        /* pointer to orphan list */
};
...
struct awbl_busctlr {
    ...
    struct awbl_buspresent buspresent;  /* 总线控制器对应的总线实例 */
};
...
struct awbl_drvinfo {
     ...
     * \retval false    探测失败
     */
    bool_t (* pfunc_drv_probe) (awbl_dev_t *p_dev);
 ...
 * \brief 硬件设备信息
 *
 * 在硬件配置文件 awbl_hwconf_usrcfg.c 中使用本结构体来描述目标系统上的硬件
 * 设备
 */
struct awbl_devhcf {

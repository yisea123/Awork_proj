...
/**
 * \addtogroup demo_if_edma_circle
...
void demo_edma_circle_entry(void){
    ...
    /* 准备一个接收传输 */
    ret = imx1050_edma_prep_dma_cyclic(rx_chan,
            (dma_addr_t)rx_circle_buf, len, period_len,DMA_DEV_TO_MEM);
    if(ret != AW_OK){
        AW_INFOF(("imx1050_edma_prep_dma_cyclic error!\r\n"));
        return ;
    }
    ...
    ret = imx1050_edma_chan_stop(tx_chan);
    if(ret != AW_OK){
        return ;
    }

    ret = imx1050_edma_chan_stop(rx_chan);
    if(ret != AW_OK){
        return ;
    }

 ...
 *      aw_sdcard_rw_sector(p_sdcard, 0, buf, 1, FALSE);
 *      memset(buf, 0x00, sizeof(buf));
 *      aw_sdcard_rw_sector(p_sdcard, 0, buf, 1, TRUE);
 *
 *      //关闭SD卡设备
 ...
 * 该接口使用时请特别注意：该接口为SD卡裸操作接口，直接操作SD卡物理块地址，不经过文件系统层
 * 使用该接口进行写操作以后将导致文件系统数据结构被破坏，直接导致数据丢失或者文件系统损坏。
 * 使用该接口操作前确保备份SD卡数据，写操作后需使用特定文件系统格式化后挂载使用文件系统操作
 */
 ...
 * \retval    FALSE   设备未插入
 */
aw_local aw_inline bool_t aw_sdcard_is_insert (const char *name)
{

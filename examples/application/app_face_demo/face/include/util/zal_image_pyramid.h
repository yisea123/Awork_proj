#ifndef ZAL_FD_UTIL_IMAGE_PYRAMID_H_
#define ZAL_FD_UTIL_IMAGE_PYRAMID_H_
...
static void ResizeImage(const zal::ImageData & src, zal::ImageData* dest) {
...
#pragma omp parallel num_threads(zal_NUM_THREADS)
  {
...
#else
		int32_t n_x_s = static_cast<int>(lf_x_scl * x);
		int32_t n_y_s = static_cast<int>(lf_y_Scl * y);
		dest_data[y * dest_width + x] = src_data[n_y_s * src_width + n_x_s];
...
class ImagePyramid {
 public:
  ...
  inline zal::ImageData image1x() {
    zal::ImageData img(width1x_, height1x_, 1);
    img.data = buf_img_;
    return img;
  }

  const zal::ImageData* GetNextScaleImage(float* scale_factor = nullptr);
 ...
 private:
  ...
  zal::ImageData img_scaled_;
};

}  // namespace fd
}  // namespace zal

#endif  // ZAL_FD_UTIL_IMAGE_PYRAMID_H_

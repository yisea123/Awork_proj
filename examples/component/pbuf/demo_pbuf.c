...
/**
 * \file
 * \brief PBUF例程，一个资源（对象或者数据包）被多个用户(线程)使用.
 *
 * - 操作步骤：
 ...
 */
aw_local void __task_func1 (void)
{
 ...
 */
aw_local void __task_func2 (void)
{
    uint8_t i;
...
{
    /* 定义任务实体，分配栈空间大小为512  */
    AW_TASK_DECL_STATIC(task_pbuf1, 512);

    /* 定义任务实体，分配栈空间大小为512  */
    AW_TASK_DECL_STATIC(task_pbuf2, 512);

 ...
 * 的传感器数据将为无效值，可以使用 AW_SENSOR_VAL_IS_VALID()宏判断哪些通道的数据
 * 读取失败了，若某一通道对应的数据为无效值，则传感器数据中的 val 值表示了标准
 * 错误码，用户可以据此判断该通道失败的原因，失败原因与标准错误码的对应关系与使
 * 用 aw_sensor_data_get() 读取单通道数据的返回值含义一致。
 *
 * \par 使用示例
 * \code
 *     #include "aw_sensor.h"
 ...
 * 有效的传感器数据，使用 AW_SENSOR_VAL_IS_VALID()宏进行有效性判定时，都会
 * 返回假，因此，没有必要使用 AW_SENSOR_VAL_IS_VALID()宏进行判断。仅需通过 val 
 * 值进行判断即可，val 值的含义与标准错误码的对应关系与使用 aw_sensor_enable() 
 * 使能单通道的返回值含义一致。特别地，若p_result 为NULL，则表明不关心每个通道
 * 使能的结果。
 */
 ...
 * 有效的传感器数据，使用 AW_SENSOR_VAL_IS_VALID()宏进行有效性判定时，都会
 * 返回假，因此，没有必要使用 AW_SENSOR_VAL_IS_VALID()宏进行判断。仅需通过 val 
 * 值进行判断即可，val 值的含义与标准错误码的对应关系与使用 aw_sensor_disable() 
 * 禁能单通道的返回值含义一致。特别地，若p_result 为NULL，则表明不关心每个通道
 * 禁能的结果。
 */

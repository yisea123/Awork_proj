...
/** \brief 控制引脚   */
typedef struct awbl_ov7725_ctr_pin {
    uint8_t  power_down_pin;
    uint8_t  reset_pin;
} awbl_ov7725_ctr_pin_t;
...
typedef struct awbl_ov7725_devinfo {
   ...
   awbl_ov7725_ctr_pin_t  ov7725_ctr_pin; /**< \brief ov7725 的控制引脚  */
...
typedef struct awbl_ov7725_dev {
    ...
    /** \brief 保存服务的头指针  */
    struct awbl_camera_controller_service *p_camera_controller_serv_head;

    uint8_t      camera_stat;             /**< \brief 摄像机当前状态  */

    uint8_t      photo_addr_stat;         /**< \brief 是否被释放  */

    uint32_t     photo_resolution;        /**< \brief 拍照模式的分辨率  */

    uint32_t     video_resolution;        /**< \brief 视频模式的分辨率  */

    uint32_t     *video_buff_addr;        /**< \brief 动态分配的视频buffer的首地址 */

    uint32_t     *photo_buff_addr;        /**< \brief 动态分配的拍照buffer的首地址 */

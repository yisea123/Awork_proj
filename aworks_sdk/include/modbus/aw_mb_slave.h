...
#define AW_MBS_TYPE_USED_HW_TMR        0x01   /**< \brief 使用硬件定时器作为字符接收间隔监测  */
#define AW_MBS_TYPE_USER_FUNCODE       0x02   /**< \brief 用户自定义功能码数配置  */
#define AW_MBS_TYPE_TCP_NUM            0x03   /**< \brief TCP模式允许建立连接数配置 */
#define AW_MBS_TYPE_ACCEPT_TCP_NUM_GET 0x04   /**< \brief 获取当前已接收的连接数 */
#define AW_MBS_TYPE_ALLOW_NEW_TCP      0x05   /**< \brief 连接达到设置值后，允许关闭旧连接来接收新连接 */
 ...
 * \param[in] slave  Modbus从站句柄
 * \param[in] type   配置类型：
 *                   - AW_MBS_TYPE_USED_HW_TMR  : 是否使用硬件定时器（默认使用软件定时器）,
 *                                                参数p_arg为int, 为1则使用硬件定时器，为0使用软件定时器;
 ...
 */
typedef bool_t (*aw_mb_tcp_accept_hook_t) (void                        *p_arg,

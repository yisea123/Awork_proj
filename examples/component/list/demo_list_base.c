...
/**
 * \file
 * \brief 链表的添加节点、删除节点、修改节点和查询节点信息例程
 *
 * - 操作步骤：
 ...
 * person_head 链表的头结点
 */
static void __list_add_node(_person_t person_head){
    uint_t i = 0;
...
static void __list_print_node(_person_t person_head){
 ...
 * person_head 链表的头结点
 */
static void __list_delete_node(_person_t person_head){

    /*  n为标记指针方便删除后继续遍历 */
    aw_list_for_each_safe(pPos,pNext,&person_head.list){
        temp = aw_list_entry(pPos,_person_t,list);
        if(temp->num == 3){
            /*  将元素删除并将删除后的节点初始化为头结点 */
 ...
 * person_head 链表的头结点
 */
static void __list_search_add_node(_person_t person_head){
...
* person_head 链表的头结点
*/
static void __list_search_replace_node(_person_t person_head){
...
{
    _person_t person_head;                           /*  定义一个双向链表头结点 */

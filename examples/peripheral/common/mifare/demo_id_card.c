...
aw_local aw_err_t mifare_card_reader_init (uint8_t dev_id)
{
    ...
    param.rx_gain           = 0x05; /* 接收增益 */
    param.cwgsn             = 0x0F; /* 无调制时段N驱动输出电导值 */
    param.modgsn            = 0x04; /* 调制时段N驱动输出电导值 */
    param.cwgsp             = 0x3F; /* 无调制时段P驱动输出电导值 */
    param.modgsp            = 0x20; /* 调制时段P驱动输出电导值 */
    param.rx_min_level      = 0x05; /* 接收译码器最小信号强度 */
    param.rx_coll_min_level = 0x05; /* 接收译码器最小信号强度 */
    param.prtl_type         = AWBL_ISO_IEC_14443_B;          /* 卡片类型 */
...
aw_local aw_err_t id_card_uid_read (uint32_t devid, id_card_info_t *p_id)
{
    ...
    ret = awbl_fm175xx_crc_set(devid, TRUE,  TRUE);
    if (AW_OK != ret) {
        AW_ERRF(("TX CRC Setting failed, errno = %d", ret));
    }

    ret = awbl_fm175xx_crc_set(devid, FALSE, TRUE);
    if (AW_OK != ret) {
...
aw_local void id_card_entry (void *p_arg)
{
    ...
    slots = 0;
    afi   = awbl_piccb_afi(AWBL_PICCB_AFI_MAIN_ALL,
                           AWBL_PICCB_AFI_SUB_ALL);
    AW_FOREVER {
        /* 卡片信息复位 */

...
/**
 * \addtogroup demo_if_edma_memcpy_sync
...
void demo_edma_memcpy_sync_entry (void){
    ...
    /* 准备一个传输  */
    ret = imx1050_edma_prep_memcpy(chan, (dma_addr_t)p_dst,
    ...
    /* 开始传输 */
    ret = imx1050_edma_chan_start_sync(chan, 20);
    if(ret != AW_OK){
        return ;
    }
    ...
    /* 释放通道   */
    ret = imx1050_edma_free_chan(chan);

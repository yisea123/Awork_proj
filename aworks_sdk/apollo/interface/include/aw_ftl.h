...
typedef enum aw_ftl_ioctl_req {
    ...
    /**
     * \brief 设置逻辑块清理的阀值。
     * 当逻辑块的脏扇区数量达到该阀值时， 会合并该逻辑块数据。取值是 0~100 代表百分值，
     * 为0 则一有脏扇区就启动清理，    为100则不会自动清理； 默认为100
     */
    AW_FTL_LOGIC_THRESHOLD,

    /**
     * \brief 这只日志块清理的阀值。
     * 当所有日志块的扇区使用数量达到该阀值时， 会合并所有日志块数据。取值是 0~100 代表百分值，
     * 为0 则一有脏扇区就启动清理，  为100则不会自动清理； 默认为85
     */
 ...
 *  aw_ftl_ioctl(p_ftl, AW_FTL_SYS_INFO_UPDATE_TIME, (void *)60000);
 *
 *  //设置系统信息的保存间隔为60000ms, 时间为0则不自动保存
 *  aw_ftl_ioctl(p_ftl, AW_FTL_SYS_INFO_TARENEW, (void *)TURE);
 *
 *  //获取设备的名称
 *  aw_ftl_ioctl(p_ftl, AW_FTL_GET_DEV_NAME, (void *)name);
 *
 *  //获取设备的扇区大小
 *  aw_ftl_ioctl(p_ftl, AW_FTL_GET_SECTOR_SIZE, (void *)&arg);
 *
 *  //获取设备的扇区数量
 *  aw_ftl_ioctl(p_ftl, AW_FTL_GET_SECTOR_COUNT, (void *)&arg);
 *
 *  //设置清理逻辑块的阀值，当逻辑块的dirty扇区达到该逻辑块扇区数量的79%时，会启动清理
 *  aw_ftl_ioctl(p_ftl, AW_FTL_LOGIC_THRESHOLD, (void *)79);
 *
 *  //设置清理日志块的阀值，当所有日志块的使用扇区达到总日志块扇区数量的87%时，会启动清理
 *  aw_ftl_ioctl(p_ftl, AW_FTL_LOG_THRESHOLD, (void *)87);
 *
 *  //删除逻辑扇区 200 的数据
 *  aw_ftl_ioctl(p_ftl, AW_FTL_SECTOR_DAT_DEL, (void *)200);
 *
 *  //格式化 ftl
 *  aw_ftl_ioctl(p_ftl, AW_FTL_DEV_FORMAT, NULL);
 *
 * \endcode
 */

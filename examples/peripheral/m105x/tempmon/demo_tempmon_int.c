...
/**
 * \addtogroup demo_if_tempmon_int
...
#include "aw_vdebug.h"
#include "aw_delay.h"
#include "driver/tempmon/awbl_imx1050_tempmon.h"
#include "aw_temp.h"
...
aw_local void __tempmon_callback (void *p_arg, uint8_t int_state)
{
    ...
    if (AWBL_IMX1050_TEMPMON_STATE_TEMP_LOW & int_state) {
        aw_kprintf("entry low temp int,temp limit 70 ~ 100 C\r\n");
    }else if(AWBL_IMX1050_TEMPMON_STATE_TEMP_HIGH & int_state){
    ...
    /* disable tempmon int */
    awbl_imx1050_tempmon_int_disable();
...
void demo_tempmon_int_entry(void){
    ...
    ret = awbl_imx1050_tempmon_limit_temp_set(AWBL_IMX1050_TEMPMON_LIMIT_TEMP_HIGH, 100000);
    if(AW_OK == ret){
        aw_kprintf("set high temperature limit is 100 C\r\n");
    }

    /* set low temperature limit is 70 ℃ */
    ret = awbl_imx1050_tempmon_limit_temp_set(AWBL_IMX1050_TEMPMON_LIMIT_TEMP_LOW, 70000);
    if(AW_OK == ret){
        aw_kprintf("set low temperature limit is 70 C\r\n");
    }

    /* connect callback func */
    ret = awbl_imx1050_tempmon_int_connect(__tempmon_callback, (void *)0);
    if(AW_OK == ret){
        aw_kprintf("connect callback func OK.\r\n");
    }

    /* enable tempmon int */
    awbl_imx1050_tempmon_int_enable();


    AW_SEMB_TAKE(temp_sem_sync, AW_SEM_WAIT_FOREVER);

    awbl_imx1050_tempmon_int_disconnect();

 ...
 *      需要修改在awbl_hwconf_xxx_emac0.h中某些配置，修改项如下：
 *      1) 若与开发板连接的PC没有DHCP服务器则将__get_dhcp_en() 函数的返回值设为FALSE表示禁能DHCP；
 *         并正确设置IP地址（和开发板应该处于统一网段）；
 *      2) 若与开发板连接的PC有DHCP服务器则将__get_dhcp_en() 函数的返回值设为TRUE表示使能DHCP，
 *         并且无需关心设置的默认IP地址。
...
/**
 * \addtogroup demo_if_modbus_tcp_master

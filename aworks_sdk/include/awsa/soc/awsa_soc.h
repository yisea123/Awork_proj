...
struct awsa_soc_dai_link {
    ...
    bool_t      symmetric_rates;  /**< \brief Symmetry requirements */
...
aw_err_t awsa_soc_platform_register (
    ...
    struct awsa_soc_dai_driver      *p_dais_drv,
    int                              num_dais,
    void                            *p_priave_data);
...
/**
 * \brief set pcm device hardware params
 *
 * \param[in] p_substream : pcm substream
 * \param[in] p_hw        : hardware params
 *
 * \retval AW_OK : success
 */
aw_err_t awsa_soc_pcm_hardware_set (
    struct awsa_soc_pcm                 *p_soc_pcm,
    const struct awsa_core_pcm_hardware *p_hw);

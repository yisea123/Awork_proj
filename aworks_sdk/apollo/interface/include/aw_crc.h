 ...
 * @{
 */
#define AW_CRC_FLAG_AUTO           1     /**< \brief 自动模式，由程序选择硬件或者软件实现 */
#define AW_CRC_FLAG_HARDWARE       2     /**< \brief 指定由硬件实现    */
#define AW_CRC_FLAG_SOFTWARE       4     /**< \brief 指定由软件实现    */
#define AW_CRC_FLAG_CREATETAB      8     /**< \brief 需要创建table表  */
/** @} */
...
typedef struct aw_crc_pattern {
    ...
    bool_t     refin;           /**< \brief 输入字节bit反序     */
    bool_t     refout;          /**< \brief 输出CRC按bit反序    */
...
typedef struct aw_crc_client {
    ...
    bool_t           isinit;      /**< \brief 是否正确初始化标志 */
    bool_t           ishardware;  /**< \brief 是否是硬件计算     */
 ...
 *
 * \retval   AW_OK       初始化成功
 * \retval   AW_ERROR    初始化失败，通常，是由于表格传入错误引起的失败，请检查表格是否传入正确
 *                       如果使能了硬件模式，则是因为没有搜索到相应CRC计算的硬件设备。
 */
 ...
 *
 * \param[in] p_crc_client  CRC模型实体，由 AW_CRC_DECL() 或 AW_CRC_DECL_STATIC() 定义的
...
/**
 * \brief 获取CRC计算结果， 用户无需直接使用该函数，应该使用宏AW_CRC_FINAL()得到最终的CRC计算结果
 *
 * \param[in] p_crc_client  CRC模型实体，由 AW_CRC_DECL() 或 AW_CRC_DECL_STATIC() 定义的
 *
 * \return crc值
 ...
 *
 * \param[in] crc        CRC对象，是后续作为宏 AW_CRC_INIT()、 AW_CRC_CAL()的第一个参数
 ...
 * \param[in] crctable   crc计算所用的table表，当前系统支持的CRC模型均提供了table表，用户
 *                       直接使用对应的表即可。需要注意的是，传入table表时，如果模型
 *                       中refin为TRUE,则需要传入后缀带ref的表。
 * \param[in] flags      CRC标志
                         - AW_CRC_FLAG_AUTO       自动模式，由程序选择硬件或者软件实现
 *                       - AW_CRC_FLAG_HARDWARE   指定由硬件计算,则table可以为NULL
 *                       - AW_CRC_FLAG_SOFTWARE   指定由软件实现
 *                       - AW_CRC_FLAG_CREATETAB  指定需要创建crc表，当系统不支持指定CRC
 *                                                模型时， 传入此标志，将会自动将crctable
 *                                                填充正确的值。
 * \retval   AW_OK       初始化成功
 * \retval   AW_ERROR    初始化失败，通常，是由于表格传入错误引起的失败，请检查表格是否传入正确
 *                       如果使能了硬件模式，则是因为没有搜索到相应CRC计算的硬件设备。

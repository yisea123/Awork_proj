 ...
 * \brief 抽象扫描器类定义
 *
 *        抽象扫描器以scan_interval的时间执行一次扫描行为, 在一次扫描行为中, 扫描scan_cnt次, 每次扫描, 依次调用
 *    回调函数管理链表中已注册的回调函数.
 *
 *        每个扫描器对象内部均维护一条回调函数管理链表, 使用者通过awbl_scan_cb_list_add/awbl_scan_cb_list_del
 *    进行增删.
 *
 * \par 使用示例
 * \code
...
struct awbl_scanner_dev {
     ...
     */
	void (*pfn_scanner_pause)(struct awbl_scanner_dev *p_scan, uint16_t pause_time);
...
aw_local aw_inline void awbl_scan_cb_list_add(struct awbl_scanner_dev     *p_scan,
		                                      struct awbl_scan_cb_handler *p_cb_handler)
{
	aw_list_add_tail(&p_cb_handler->list_elm, &p_scan->cb_list_head);
}

aw_local aw_inline void awbl_scan_cb_list_del(struct awbl_scan_cb_handler *p_cb_handler)
{
...
void awbl_scanner_dev_ctor(struct awbl_scanner_dev *p_scan, struct awbl_scanner_param *p_par);

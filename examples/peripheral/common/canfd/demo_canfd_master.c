 ...
 * \file
 * \brief CANFD控制器演示例程
 * \note  本例程作为控制端，被控制端可利用 demo_canfd_slave.c。
 *
 * - 操作步骤：
 ...
 *      收发器再接到需要通讯的设备或通道CANFD的收发器上 H=>H L=>L。
 *   3. 打开串口调试终端，波特率115200-8-N-1。
 *
 * - 实验现象：
 *   1. 按下按键KEY_1，本通道会发送控制LED报文，DATA每bit代表一个灯状态，关闭所有灯
 *      (标准帧 ID=0X01 DATA=0X00 0X00 0X00 0X00 0X00 0X00 0X00 0X00)。
 *   2. 松开按键KEY_1，本通道会发送控制LED报文，DATA每bit代表一个灯状态，打开所有灯
 *      (标准帧 ID=0X01 DATA=0XFF 0XFF 0XFF 0XFF 0XFF 0XFF 0XFF 0XFF)。
 *   3. 收到LED控制应答报文，蜂鸣器会响起
 *      (标准帧 ID=0X81 DATA=0X00 0X00 0X00 0X00 0X00 0X00 0X00 0X00)。
 *   4. 串口打印相关调试信息。
 *
 * \par 源代码
 * \snippet demo_canfd_master.c src_fd_can
 *
 * \internal
...
#define __CAN_RECV_TASK_PRIO       7     /* CAN报文接收任务优先级 */
#define __CAN_SEND_TASK_PRIO       8     /* CAN报文发送任务优先级 */
...
aw_local void __key_proc_cb (aw_input_event_t *p_input_data, void *p_usr_data)
{
    ...
    if (p_key_data->key_code == KEY_1) {
        if (p_key_data->key_state) {
            ...
            msg.can_msg.flags = AW_CAN_MSG_FLAG_FD_CTRL | AW_CAN_MSG_FLAG_BRS;
...
static void __can_send_task_entry (void *p_arg)
{
    ...
    aw_can_fd_msg_t     msg[__CAN_MSG_CNT]; /* CAN信息缓冲区 */

    AW_RNGBUF_DEFINE(rngbuf, aw_can_fd_msg_t, __CAN_MSG_CNT);
    ...
    AW_FOREVER {
        ...
        send_cnt = aw_rngbuf_nbytes(&rngbuf) / sizeof(aw_can_fd_msg_t);
        if (send_cnt > 0) {

 ...
 *
 * \note 这个接口从EIO通道读取len个字节数据，在timeout时间内未完成此操作时停止操作并返回
 *       已经读取到的字节数。
 */
 ...
 * \retval -AW_ENOMEM  初始化信号量失败
 *
 * \note 这个接口向EIO通道写入len个字节数据，在timeout内时间未完成此操作时停止操作并返回
 *       已经写入的字节数。
 */
 ...
 * \retval -AW_EINVAL  参数错误
 *
 * \note 这个接口向通道申请一次异步传输，如果pfn_complete不为空，此传输完成后会调用
 *       pfn_complete回调函数，如果pfn_complete为空，此传输完成后会将传输完的数据和结果
 *       存放在该通道内，用户使用aw_eio_reclaim可以从通道回收该传输操作。
 */
 ...
 * \retval -AW_EINVAL 参数错误
 *
 * \note 这个接口与aw_eio_issue结合使用，用于回收一个异步传输操作，异步传输操作的申请与回
 *       收遵循FIFO原则，先申请的操作先被回收后申请的操作后被回收。aw_eio_reclaim接口返
 *       回后pp_buf指向回收到的缓冲区首地址，其他返回数据分别在p_opt，p_ret_len，
 *       p_status中。调用此接口后timeout时间内如果没有回收到传输操作时，如果abort为TRUE
 *       时会强制终止正在传输的数据操作，如果有该操作，返回该操作，abort为FALSE会一直等到
 *       该操作完成时才返回。
 */
 ...
 * \note 这个接口向通道申请一次异步传输，如果pfn_complete不为空，此传输完成后会调用
 *       pfn_complete回调函数，如果pfn_complete为空，此传输完成后会将传输完的数据和结果
 *       存放在该通道内，用户使用aw_eio_pbuf_reclaim可以从通道回收该传输操作。
 */
 ...
 * \retval -AW_EINVAL 参数错误
 *
 * \note 这个接口与aw_eio_pbuf_issue结合使用，用于回收一个异步传输操作，异步传输操作的申
 *       请与回收遵循FIFO原则，先申请的操作先被回收后申请的操作后被回收。这个接口返回后
 *       pp_buf指向回收到的缓冲区首地址，其他返回数据分别在p_opt，p_ret_len，p_status中
 *       。调用此接口后timeout时间内如果没有回收到传输操作时，如果abort为TRUE时会强制终止
 *       正在传输的数据操作，如果有该操作，返回该操作，abort为FALSE会一直等到该操作完成时
 *       才返回。
 */
 ...
 * \retval 无
 *
 * \note 由EIO设备驱动初始化的时候调用添加设备通道，先创建设备，后添加通道，这个接口在
 *       aw_eio_dev_create后调用
 */
 ...
 * \retval NULL     通道为空
 *
 * \note 这个接口为EIO向下接口，当EIO调用start服务启动通道传输之后，驱动会从该通道不断获取数据包
 *       进行传输，传输完会将完成的数据包提交给EIO,直到通道为空停止传输
 */
 ...
 *
 * \note 这个接口为EIO向下接口，获取指定数据包的一块数据缓冲区，一个数据包p_pkt的缓冲区可
 *       能是多个且非连续的，使用这个接口不断获取数据包的数据缓冲区分块并传输，直到数据包中
 *       已经没有缓冲区时表示这一包数据已经完成传输操作，可以调用aw_eio_dev_pkt_done提交
 *       这一包数据
 */
 ...
 * \retval 无
 *
 * \note 这个接口为EIO向下接口，EIO设备驱动完成一个包的传输后调用这个接口提交数据包给EIO
 */

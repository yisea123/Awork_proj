...
void demo_nand_fs_entry (void)
{
     ...
     * note: mtd的名字("/dev/mtd0")在 nandflash 的硬件配置(awbl_hwconf_xxxxxx_nandflash.h)
     *       里查找
     * note: 若nand没有被格式化，那么需要格式化；若格式化过，就不需要此操作
     */
    if ((err = aw_make_fs("/dev/mtd0", "lffs", &fmt)) != AW_OK) {
        AW_INFOF(("failed: %d\n", err));
    } else {
        AW_INFOF(("OK\n"));
    }
     ...
     */
    if ((err = aw_mount("/mnt", "/dev/mtd0", "lffs", AW_OK)) == AW_OK) {
        AW_INFOF(("OK\n"));
    }

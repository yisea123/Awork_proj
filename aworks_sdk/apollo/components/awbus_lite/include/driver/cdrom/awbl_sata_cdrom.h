...
#define AWBL_SATA_CDROM_FLAG_LBA            (0X0001) /* 该设备支持LBA寻址 */
#define AWBL_SATA_CDROM_FLAG_LBA48          (0X0002) /* 该设备支持LBA48寻址 */
#define AWBL_SATA_CDROM_FLAG_NCQ            (0X0004) /* 该设备支持NCQ */
#define AWBL_SATA_CDROM_FLAG_DMA            (0X0008) /* 该设备支持DMA */
...
typedef struct awbl_sata_cdrom_identify {
    ...
    uint32_t lba;               /* 逻辑块地址 */
    uint32_t blk_size;          /* 块的大小 Byte */
    uint32_t sectors_per_frame; /* 每帧所占扇区数 */
    uint32_t cdrom_capacity;    /* 磁盘容量 MByte */

    awbl_sata_cdrom_flag_t flag; /* 该设备支持的功能 */
...
struct awbl_sata_cdrom_dev {
    ...
    bool_t   is_found_disk; /** \brief 是否检测到光盘 */

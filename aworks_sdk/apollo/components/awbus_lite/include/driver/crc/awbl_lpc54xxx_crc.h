...
/** \brief 模式设置寄存器位功能定义 */
#define CRC_MODE_POLY_CRC_32      (3)         /* ploy = 0x04C11DB7 */
#define CRC_MODE_POLY_CRC_16      (1)         /* poly = 0x8005     */
#define CRC_MODE_POLY_CRC_CCITT   (0)         /* poly = 0x1021     */

#define CRC_MODE_BIT_RVS_WR       (1 << 2)    /* Bit order reverse for CRC_WR_DATA (per byte) */
#define CRC_MODE_CMPL_WR          (1 << 3)    /* Data 1's complement for CRC_WR_DATA */
                                              /* 正数 = 原码，负数 = 反码             */
                                              /* 2's complment 2的补码               */
                                              /* 正数 = 原码，负数 = 反码 + 1         */
                                              
#define CRC_MODE_BIT_RVS_SUM      (1 << 4)    /* CRC sum bit order                   */
#define CRC_MODE_CMPL_SUM         (1 << 5)    /* CRC sum complement                  */

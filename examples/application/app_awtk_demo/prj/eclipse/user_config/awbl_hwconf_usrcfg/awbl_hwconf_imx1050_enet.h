...
#include "driver/net/awbl_imx1050_enet.h"
...
aw_local void __imx1050_enet_plfm_init (void)
{
    ...
    if (aw_gpio_pin_request(AWBL_IMX1050_ENET_NAME, enet_gpios, AW_NELEMENTS(enet_gpios)) == AW_OK) {
        /* 先为PHY芯片提供50Mhz时钟输入 */
...
/** \brief 判断以太网地址是否全为0 */
aw_local inline bool_t is_zero_ether_addr (const uint8_t *addr)
{
    uint8_t val = 0;
...
aw_local inline bool_t is_multicast_ether_addr (const uint8_t *addr)
{
    if (0x1 & addr[0]) {
        return TRUE;
    }
...
/** \brief 判断以太网地址是否合法 */
aw_local inline bool_t is_valid_ether_addr (const uint8_t *addr)
{
...
aw_local aw_err_t __get_macaddr (uint8_t *p_mac_buf, int buf_len)
{
    ...
    aw_nvram_get("otp_zy_mac", 0, (char *) data, 0, sizeof(data));
    for (i = 0; i < 6; i++) {
...
/* 设备信息 */
aw_local aw_const struct awbl_imx1050_enet_devinfo __g_imx1050_enet_devinfo = {
	"eth0",
	&enet_info,
...
/* 设备实例内存静态分配 */
aw_local struct awbl_imx1050_enet_dev __g_imx1050_enet_dev;

#define AWBL_HWCONF_IMX1050_ENET                      \
    {                                                 \
        AWBL_IMX1050_ENET_NAME,                       \
        0,                                            \
        AWBL_BUSID_PLB,                               \
        0,                                            \
